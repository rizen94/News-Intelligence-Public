      ...
      const response = await apiService.getMarketPatterns(
        {
          ...filters,
          patternType:
            filters.patternType !== 'all' ? filters.patternType : undefined,
    ...
    <Box sx={{ p: 3 }}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={e =>
                  handleFilterChange('patternType', e.target.value)
                }
                label='Pattern Type'
              >
      ...
      {!loading && !error && (
        <>
          <Box sx={{ mb: 2 }}>
            <Chip
              label='API Endpoint: /api/finance/finance/market-patterns'
              color='info'
              size='small'
            />
          </Box>
          ...
          {patterns.length === 0 ? (
            <Alert severity='info'>
              No market patterns found. The backend API endpoint needs to be
              implemented.
            </Alert>
          ) : (
                  ...
                  <Accordion>
                    <AccordionSummary expandIcon={<ExpandMore />}>
                      ...
                      >
                        ...
                        >
                          <Chip
                            label={`${(pattern.confidence_score * 100).toFixed(
                              0
                            )}% confidence`}
                            size='small'
                            color={
                          ...
                          {pattern.market_impact && (
                            <Chip
                              label={`${
                                pattern.market_impact > 0 ? '+' : ''
                              }${pattern.market_impact.toFixed(2)}% impact`}
                              size='small'
                              color={
                                pattern.market_impact >= 0 ? 'success' : 'error'
                              }
                              icon={
                                pattern.market_impact >= 0 ? (
                                  <TrendingUp />
                        ...
                        <Grid item xs={12} md={6}>
                          ...
                          >
                            ...
                            <Typography variant='body2'>
                              ...
                              {pattern.pattern_duration_days || 0} days
                            </Typography>
                            <Typography variant='body2'>
                              <strong>Strength:</strong>{' '}
                              {(pattern.pattern_strength * 100).toFixed(0)}%
                            </Typography>
                            {pattern.affected_companies &&
                              pattern.affected_companies.length > 0 && (
                                <Box>
                                  ...
                                  >
                                    {pattern.affected_companies.map(
