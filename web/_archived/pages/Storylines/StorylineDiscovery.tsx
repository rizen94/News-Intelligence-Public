...
const StorylineDiscovery: React.FC = () => {
  ...
  /** When true, kick off background full-backlog discovery (sync would time out). */
  const [useFullBacklog, setUseFullBacklog] = useState(false);
  ...
  const handleDiscover = async () => {
    try {
      ...
      const response = await apiService.discoverStorylines({
        ...(useFullBacklog ? { async: true } : { hours }),
        minArticles: minClusterSize,
        minSimilarity: similarityThreshold,
        save: autoSave,
      });
      if (response?.async && response?.success) {
        setSuccess(
          response.message ||
            `Background discovery started (task ${response.task_id}). ` +
              `Estimated ~${response.estimated_duration_minutes ?? 45} minutes — ` +
              'refresh Stories later; this page will not list clusters from a background run.'
        );
        setDiscoveredStorylines([]);
            ...
            <CardContent>
              ...
              <Box sx={{ mb: 3 }}>
                <FormControlLabel
                  ...
                  label='Full historical backlog (background job; default off)'
                />
                {useFullBacklog ? (
                  <Typography variant='caption' color='text.secondary' display='block' sx={{ mt: 1 }}>
                    Full backlog can take tens of minutes. Run Discovery queues a background job
                    instead of waiting in the browser.
                  </Typography>
                ) : (
                  <>
