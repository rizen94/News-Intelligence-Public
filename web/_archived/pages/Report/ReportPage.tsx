...
function patternDiscoverySummary(p: PatternDiscovery): string {
  ...
  const names = (p.entity_names ?? []).filter(Boolean).slice(0, 3);
  const d =
    p.data != null && typeof p.data === 'object'
      ? (p.data as Record<string, unknown>)
      : {};
  const ctx =
    typeof d.context_count === 'number'
      ? d.context_count
      : typeof d.context_count === 'string' && d.context_count
        ? d.context_count
        : null;
  const date = typeof d.date === 'string' ? d.date : null;
  const relation =
    typeof d.relation === 'string' ? d.relation.replace(/_/g, ' ') : null;
  const sourceType =
    typeof d.source_type === 'string' ? d.source_type : null;

  const bits: string[] = [];
  if (names.length) bits.push(names.join(' · '));
  if (relation) bits.push(relation);
  if (date) bits.push(date);
  if (sourceType) bits.push(`via ${sourceType}`);
  if (ctx != null) bits.push(`${ctx} contexts`);
  if (!bits.length && Object.keys(d).length) {
    bits.push(
      Object.keys(d)
        .slice(0, 2)
        .map(k => `${k}: ${String(d[k]).slice(0, 40)}`)
        .join(' · ')
    );
  }
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        <Box>
          ...
          <Typography variant='body2' color='text.secondary' sx={{ mb: 1.5 }}>
            Higher-signal pattern discoveries (multi-context / behavioral /
            temporal). Open Search for the full graph including weak
            co-mentions.
          </Typography>
          {patternsLoading ? (
