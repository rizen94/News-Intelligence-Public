...
function DomainRAG() {
  ...
  async function loadDomainKnowledge() {
    ...
    try {
      const [entitiesRes, termsRes, sourcesRes] = await Promise.all([
        fetch(`/api/${domain}/rag/knowledge/entities`),
        fetch(`/api/${domain}/rag/knowledge/terminology`),
        fetch(`/api/${domain}/rag/knowledge/sources`),
      ]);
  ...
  async function handleQuery() {
    ...
    try {
      let response;
      if (crossDomain) {
        response = await fetch('/api/rag/cross-domain', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query, hours, max_chunks: 10 }),
        });
      } else {
        response = await fetch(`/api/${domain}/rag/query`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query, hours, max_chunks: 10 }),
        });
