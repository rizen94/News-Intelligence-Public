...
export default function ProcessedDocumentsPage() {
  ...
  const handleIngestFromConfig = async () => {
    ...
    try {
      ...
      setMessage({
        text: `Inserted ${res.inserted} document(s).${
          res.errors?.length
            ? ` Errors: ${res.errors.slice(0, 3).join('; ')}`
            : ''
        }`,
        severity:
          res.errors?.length && res.inserted === 0 ? 'error' : 'success',
      });
      await load();
    } catch (e) {
      setMessage({
        text: (e as Error)?.message ?? 'Ingest failed',
        severity: 'error',
      });
    } finally {
      setIngesting(false);
  ...
  const handleProcessNow = async () => {
    ...
    try {
      ...
      setMessage({
        text: (e as Error)?.message ?? 'Processing failed',
        severity: 'error',
      });
    } finally {
      setProcessing(false);
  ...
  const handleAddSubmit = async () => {
    ...
    try {
      ...
      if (res?.document_id) {
        ...
        setMessage({
          text: `Document #${res.document_id} added.`,
          severity: 'success',
        });
        await load();
      } else {
  ...
  return (
    <Box>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            <TextField
              ...
              onChange={e =>
                setAddForm(f => ({ ...f, source_url: e.target.value }))
              }
              placeholder='https://...'
              required
              fullWidth
            ...
            <TextField
              ...
              onChange={e =>
                setAddForm(f => ({ ...f, source_type: e.target.value }))
              }
              fullWidth
              size='small'
              placeholder='e.g. government, think_tank'
            />
            <TextField
              label='Source name'
              value={addForm.source_name}
              onChange={e =>
                setAddForm(f => ({ ...f, source_name: e.target.value }))
              }
              fullWidth
              size='small'
            />
            <TextField
              ...
              onChange={e =>
                setAddForm(f => ({ ...f, document_type: e.target.value }))
              }
              fullWidth
              size='small'
              placeholder='e.g. report, analysis'
            />
            <TextField
              ...
              onChange={e =>
                setAddForm(f => ({ ...f, publication_date: e.target.value }))
              }
      ...
      <Typography variant='body2' color='text.secondary' sx={{ mb: 1.5 }}>
        Same list in every domain. Sources: <strong>Ingest from config</strong>{' '}
        (<code>document_sources.ingest_urls</code>),{' '}
        <strong>Add document</strong>, or automated collection (CRS, GAO, CBO,
        arXiv) every 6 hours.
      </Typography>
      {loadError && (
        <Alert
        ...
        <Card variant='outlined'>
          <CardContent>
            ...
            <Box component='ul' sx={{ m: 0, pl: 2.5, color: 'text.secondary' }}>
              <li>
                <strong>Ingest from config</strong> — add URLs to{' '}
                <code>document_sources.ingest_urls</code> in{' '}
                <code>orchestrator_governance.yaml</code>, then click the button
                above.
              </li>
              <li>
                <strong>Add document</strong> — add a single PDF URL (e.g. a CRS
                or GAO report). The pipeline will process it when automation
                runs (or trigger via API).
              </li>
              <li>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {items.map(doc => (
            <Card key={doc.id} variant='outlined'>
              <CardActionArea
                onClick={() =>
                  navigate(`/${domain}/investigate/documents/${doc.id}`)
                }
              >
