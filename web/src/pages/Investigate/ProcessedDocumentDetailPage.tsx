...
export default function ProcessedDocumentDetailPage() {
  ...
  const sections: Section[] = Array.isArray(doc.extracted_sections)
    ? doc.extracted_sections
    : [];
  const findings: Finding[] = Array.isArray(doc.key_findings)
    ? doc.key_findings
    : [];
  const entities: Entity[] = Array.isArray(doc.entities_mentioned)
      ...
      >
        ...
        <Card variant='outlined'>
          <CardContent>
            ...
            entities.length === 0 ? (
              <Typography variant='body2' color='text.secondary'>
                No extractions yet. Process this document from the list (or run
                batch process) to extract sections and findings.
              </Typography>
            ) : (
