...
export default function NarrativeThreadsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await contextCentricApi
        .getNarrativeThreads({ domain_key: domain || undefined, limit: 100 })
        .catch(() => ({ items: [] }));
      setItems(res?.items ?? []);
    } finally {
      setLoading(false);
  ...
  const handleBuild = async () => {
    ...
    setBuilding(true);
    try {
      const res = await contextCentricApi.buildNarrativeThreads({
        domain_key: domain,
        limit: 50,
      });
      setMessage({
        text: `Built ${res.built} thread(s).${
          res.errors?.length
      ...
      setMessage({
        text: (e as Error)?.message ?? 'Build failed',
        severity: 'error',
      });
    } finally {
      setBuilding(false);
  ...
  const handleSynthesize = async () => {
    ...
    try {
      const res = await contextCentricApi.synthesizeNarrativeThreads(
        domain ? { domain_key: domain } : undefined
      );
  ...
  return (
    <Box>
      ...
      <Typography variant='body2' color='text.secondary' sx={{ mb: 1.5 }}>
        Threads are built from storylines (summary + linked articles). Build for
        the current domain, then run Synthesize to combine thread summaries.
      </Typography>
      {loading ? (
        ...
        <Card variant='outlined'>
          <CardContent>
            <Typography color='text.secondary'>
              No narrative threads yet. Use{' '}
              <strong>Build for {domain || 'domain'}</strong> to create threads
              from storylines in this domain.
            </Typography>
          </CardContent>
        </Card>
      ) : (
