/**
 * Discover — Latest contexts, entity browser, event timeline.
 */
...
export default function DiscoverPage() {
  ...
  // Load contexts first (visible tab), then entities — faster first paint
  useEffect(() => {
    let cancelled = false;
    setEntityError(null);
    const params = { domain_key: domain, limit: INITIAL_PAGE_SIZE, brief: true };

    const load = async () => {
      setContextsLoading(true);
      setEntitiesLoading(true);
      try {
        const ctxRes = await contextCentricApi.getContexts(params).catch(() => ({ items: [] as Context[] }));
        if (cancelled) return;
        setContexts(ctxRes?.items ?? []);
        setContextsLoading(false);

        let entRes: { items?: EntityProfile[] } = { items: [] };
        try {
          entRes = await contextCentricApi.getEntityProfiles(params);
        } catch (err: unknown) {
          const detail = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
          if (!cancelled) setEntityError(detail ?? (err as Error)?.message ?? 'Request failed');
        }
        if (cancelled) return;
  ...
  const handleSyncEntities = async () => {
    ...
    try {
      ...
      if (res.success && res.created_by_domain) {
        ...
        setSyncMessage(total > 0 ? `Synced: ${total} new entity profile(s) for ${domain}.` : 'No new entities to sync (entity_canonical may be empty).');
        if (total > 0) {
          const entRes = await contextCentricApi.getEntityProfiles({ domain_key: domain, limit: INITIAL_PAGE_SIZE, brief: true }).catch(() => ({ items: [] as EntityProfile[] }));
          setEntities(entRes?.items ?? []);
        }
      } else {
  ...
  const handleSyncContexts = async () => {
    ...
    try {
      ...
      if (res.success && res.contexts_created_by_domain) {
        ...
        setContextSyncMessage(created > 0 ? `Created ${created} context(s) for ${domain}.` : 'No new contexts (all articles may already have contexts, or there are no articles for this domain).');
        if (created > 0) {
          const ctxRes = await contextCentricApi.getContexts({ domain_key: domain, limit: INITIAL_PAGE_SIZE, brief: true }).catch(() => ({ items: [] as Context[] }));
          setContexts(ctxRes?.items ?? []);
        }
      } else {
        ...
        <Box>
          {contextsLoading ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {[0, 1, 2].map((i) => <Skeleton key={i} variant="rectangular" height={120} sx={{ borderRadius: 1 }} />)}
            </Box>
          ) : contexts.length === 0 ? (
            <Card variant="outlined">
              <CardContent>
                <Typography color="text.secondary" sx={{ mb: 2 }}>
                  No contexts yet for {domain}. Politics and finance use the same pipeline; contexts come from articles in this domain.
                </Typography>
                ...
                <Button
                  variant="outlined"
                  size="small"
                  onClick={handleSyncContexts}
                  disabled={contextSyncLoading}
                >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {contexts.map((c) => {
                ...
                const date = c.created_at ? new Date(c.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : null;
                return (
            ...
            {entitiesLoading ? (
              ...
              <Box>
                <Typography color="text.secondary" paragraph>
                  No entity profiles yet. Entities are created from <strong>entity_canonical</strong> (filled by article entity extraction). Run sync to copy them into the intelligence layer.
                </Typography>
                {syncMessage?.toLowerCase().includes('404') && (
                  <Alert severity="warning" sx={{ mb: 2 }}>
                    Context-centric API not found (404). Use the <strong>server root</strong> as API base: <code>http://localhost:8000</code> (or your backend host). In <code>.env</code> set <code>VITE_API_URL=http://localhost:8000</code> then restart the dev server; or leave it unset to use the Vite proxy. Reload and try Sync again.
                  </Alert>
                )}
              ...
              <List dense>
                {entities.map((p) => (
                  <ListItemButton
                    key={p.id}
                    onClick={() => navigate(`/${domain}/investigate/entities/${p.id}`)}
                  >
                    <ListItemText
