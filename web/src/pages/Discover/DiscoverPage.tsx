...
export default function DiscoverPage() {
  ...
  const [contextSyncMessage, setContextSyncMessage] = useState<string | null>(
    null
  );
  ...
  const contextPageCount = Math.max(
    1,
    Math.ceil(contextTotal / PAGE_SIZE) || 1
  );
          ...
          {contextsLoading ? (
            ...
            <Card variant='outlined'>
              <CardContent>
                <Typography color='text.secondary' sx={{ mb: 2 }}>
                  No contexts yet for {domain}. Politics and finance use the
                  same pipeline; contexts come from articles in this domain.
                </Typography>
                <Typography
                  variant='body2'
                ...
                >
                  {contextSyncLoading
                    ? 'Syncing…'
                    : 'Sync contexts for this domain'}
                </Button>
                {contextSyncMessage && (
                  <Alert
                    severity='info'
                    sx={{ mt: 2 }}
                    onClose={() => setContextSyncMessage(null)}
                  >
                    {contextSyncMessage}
                  </Alert>
                )}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Typography variant='body2' color='text.secondary'>
                ...
                {Math.min(contextPage * PAGE_SIZE, contextTotal)} of{' '}
                {contextTotal} contexts
              </Typography>
              {contexts.map(c => {
                ...
                return (
                  <Card key={c.id} variant='outlined'>
                    <CardActionArea
                      onClick={() =>
                        navigate(`/${domain}/discover/contexts/${c.id}`)
                      }
                    >
            ...
            {entitiesLoading ? (
              ...
              <Box>
                <Typography color='text.secondary' paragraph>
                  No entity profiles yet. Entities are created from{' '}
                  <strong>entity_canonical</strong> (filled by article entity
                  extraction). Run sync to copy them into the intelligence
                  layer.
                </Typography>
                {syncMessage?.toLowerCase().includes('404') && (
                  <Alert severity='warning' sx={{ mb: 2 }}>
                    Context-centric API not found (404). Use the{' '}
                    <strong>server root</strong> as API base:{' '}
                    <code>http://localhost:8000</code> (or your backend host).
                    In <code>.env</code> set{' '}
                    <code>VITE_API_URL=http://localhost:8000</code> then restart
                    the dev server; or leave it unset to use the Vite proxy.
                    Reload and try Sync again.
                  </Alert>
                )}
                <Typography
                  variant='caption'
                ...
                >
                  API base:{' '}
                  {getCurrentApiUrl() || '(dev proxy → localhost:8000)'}
                </Typography>
                <Button
                  variant='outlined'
              ...
              <>
                <Typography
                  ...
                  sx={{ mb: 1 }}
                >
                  Showing {(entityPage - 1) * PAGE_SIZE + 1}–
                  {Math.min(entityPage * PAGE_SIZE, entityTotal)} of{' '}
