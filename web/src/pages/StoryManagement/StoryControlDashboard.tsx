...
interface FeedbackLoopStatus {
  is_running: boolean;
  stories_being_tracked: number;
  articles_processed_today: number;
  rag_enhancements_triggered: number;
}

interface WeeklyDigest {
  total_articles_analyzed: number;
  new_stories_suggested: number;
...
const StoryControlDashboard: React.FC = () => {
  ...
  const fetchStories = async () => {
    try {
      ...
      showError(`Failed to load stories: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchFeedbackLoopStatus = async () => {
    try {
      const response = await apiService.getFeedbackLoopStatus();
  ...
  const fetchWeeklyDigest = async () => {
    try {
      const response = await apiService.getRecentDigests(1);
  ...
  const handleStartFeedbackLoop = async () => {
    try {
      ...
      showLoading('Starting feedback loop...');

      await apiService.startFeedbackLoop();

      showSuccess('Feedback loop started successfully!');
      await fetchFeedbackLoopStatus();
    } catch (err: any) {
      console.error('Error starting feedback loop:', err);
      showError(`Failed to start feedback loop: ${err.message}`);
    } finally {
  ...
  const handleStopFeedbackLoop = async () => {
    try {
      ...
      showLoading('Stopping feedback loop...');

      await apiService.stopFeedbackLoop();

      showSuccess('Feedback loop stopped successfully!');
      await fetchFeedbackLoopStatus();
    } catch (err: any) {
      console.error('Error stopping feedback loop:', err);
      showError(`Failed to stop feedback loop: ${err.message}`);
    } finally {
  ...
  const handleGenerateDigest = async () => {
    try {
      ...
      showLoading('Generating weekly digest...');

      await apiService.generateWeeklyDigest();

      showSuccess('Weekly digest generated successfully!');
      await fetchWeeklyDigest();
    } catch (err: any) {
      console.error('Error generating digest:', err);
      showError(`Failed to generate digest: ${err.message}`);
    } finally {
                ...
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={e =>
                      setStoryForm(prev => ({
                        ...prev,
                        priority_level: e.target.value as number,
                      }))
                    }
                  >
