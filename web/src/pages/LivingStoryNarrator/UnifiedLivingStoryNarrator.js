...
const UnifiedLivingStoryNarrator = () => {
  ...
  const handleRunPreprocessing = async() => {
    ...
    setError(null);
    try {
      const response = await newsSystemService.runPreprocessing({
        batch_size: 20,
      });
      if (response.success) {
        setSuccess(
          `Preprocessing completed: ${response.data.articles_processed} articles processed`,
        );
        loadPreprocessingStatus();
        loadMasterArticles();
      } else {
            ...
            <div className='unified-stat-card unified-fade-in'>
              <div className='unified-stat-card-content'>
                ...
                <div
                  ...
                  style={{ color: '#2e7d32' }}
                >
                  {preprocessingStatus.processing_statistics?.tags_extracted ||
                    0}
                </div>
      ...
      {narratorStatus?.next_scheduled_tasks && (
        <div className='unified-section'>
          <div className='unified-content-card unified-fade-in'>
            ...
            <div className='unified-content-body'>
              <List dense>
                {Object.entries(narratorStatus.next_scheduled_tasks).map(
                  ([task, time]) => (
                    <ListItem key={task}>
                      <ListItemIcon>
        ...
        <div className='unified-content-card unified-fade-in'>
          ...
          <div className='unified-content-body'>
            {dailyDigests.length > 0 ? (
              ...
              <div className='unified-content-text'>
                No daily digests available yet. Generate one using the manual
                actions above.
              </div>
            )}
          </div>
        ...
        <div className='unified-content-card unified-fade-in'>
          ...
          <div className='unified-content-body'>
            {masterArticles.length > 0 ? (
              ...
              <div className='unified-content-text'>
                No master articles available yet. Run preprocessing to create
