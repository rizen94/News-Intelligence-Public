...
import ProvenancePanel, { storylineProvenanceRows } from '../../components/ProvenancePanel/ProvenancePanel';
...
const StorylineDetail = () => {
  ...
  const [detailDepth, setDetailDepth] = useState<'narrative' | 'structured' | 'raw'>('narrative');
  ...
  useEffect(() => {
    ...
    apiService
      ...
      .then((r) => {
        if (cancelled) return;
        if (r?.success && r.data) setStoryAudit(r.data);
        else setStoryAuditErr(r?.error || 'Audit request failed');
      })
      .catch((e) => {
  ...
  const checkCachedSynthesis = async() => {
    try {
      const response = await fetch(`/api/${effectiveDomain}/synthesis/storyline/${id}/cached`);
      if (response.ok) {
  ...
  const handleGenerateSynthesis = async(regenerate = false) => {
    try {
      ...
      const response = await fetch(`/api/${effectiveDomain}/synthesis/storyline/${id}`, {
        method: 'POST',
  ...
  return (
    <Box>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Card>
            <CardContent>
              ...
              <Paper variant='outlined' sx={{ p: 2, mb: 2, bgcolor: 'grey.50' }}>
                ...
                {detailDepth === 'narrative' && (
                  <Typography variant='body2' color='text.secondary' sx={{ mt: 1 }}>
                    Reader-first view: editorial summary, storyline analysis, and synthesis.
                  </Typography>
                )}
                {detailDepth === 'structured' && (
                  <Typography variant='body2' color='text.secondary' sx={{ mt: 1 }}>
                    Audit view: storyline status, article counts, timeline signals, and event-level metadata.
                  </Typography>
                )}
                {detailDepth === 'raw' && (
                  <Accordion disableGutters sx={{ mt: 1, bgcolor: 'transparent', boxShadow: 'none', '&:before': { display: 'none' } }}>
                    ...
                    <AccordionDetails>
                      <Typography variant='caption' sx={{ whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>
                        {JSON.stringify({ storyline, storyAudit, timelineData }, null, 2)}
                      </Typography>
                    </AccordionDetails>
                  </Accordion>
                )}
              ...
              {storyline.editorial_document && typeof storyline.editorial_document === 'object' && (
                <Paper variant="outlined" sx={{ p: 2, mb: 2, bgcolor: 'grey.50' }}>
                  ...
                  {(() => {
                    ...
                    const has5W1H = ed.who != null || ed.what != null || ed.when != null || ed.where != null;
                    if (has5W1H && (ed.lede || ed.who?.length || ed.what?.length)) {
                      return (
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                          {ed.lede && <Typography variant="body1" fontWeight={500}>{ed.lede}</Typography>}
                          {ed.who && ed.who.length > 0 && (
                            <Box>
                              <Typography variant="caption" color="text.secondary" fontWeight={600}>Who</Typography>
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75, mt: 0.5 }}>
                                {ed.who.map((w, i) => (
                                  <Typography key={i} variant="body2">{(w.name || '').trim()}{(w.role || w.background) ? ` — ${(w.role || w.background).slice(0, 80)}` : ''}</Typography>
                                ))}
                              </Box>
                            </Box>
                          )}
                          {ed.what && ed.what.length > 0 && (
                            <Box component="ul" sx={{ pl: 2, m: 0 }}>
                              <Typography variant="caption" color="text.secondary" fontWeight={600}>What</Typography>
                              {ed.what.slice(0, 5).map((w, i) => <Typography key={i} variant="body2" component="li">{w}</Typography>)}
                            </Box>
                          )}
                          {ed.when && ed.when.length > 0 && (
                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                              <Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ width: '100%' }}>When</Typography>
                              {ed.when.slice(0, 5).map((t, i) => <Chip key={i} size="small" label={t} variant="outlined" />)}
                            </Box>
                          )}
                          {ed.where && ed.where.length > 0 && (
                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                              <Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ width: '100%' }}>Where</Typography>
                              {ed.where.slice(0, 4).map((w, i) => <Chip key={i} size="small" label={w} variant="outlined" />)}
                            </Box>
                          )}
                          {(ed.why || ed.how) && <Typography variant="body2" color="text.secondary">{[ed.why, ed.how].filter(Boolean).join(' ')}</Typography>}
                          {ed.outlook && <Typography variant="body2" sx={{ fontStyle: 'italic' }}>Outlook: {ed.outlook}</Typography>}
                        </Box>
                      );
                    }
                    return (
                      <Box>
                        {ed.lede && <Typography variant="body1" sx={{ mb: 1 }}>{ed.lede}</Typography>}
                        {(ed.developments || []).length > 0 && (
                          <Box component="ul" sx={{ pl: 2, m: 0 }}>
                            {ed.developments.map((d, i) => <Typography key={i} variant="body2" component="li">{d}</Typography>)}
                          </Box>
                        )}
                        {ed.analysis && <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>{ed.analysis}</Typography>}
                        {ed.outlook && <Typography variant="body2" sx={{ mt: 1, fontStyle: 'italic' }}>{ed.outlook}</Typography>}
                      </Box>
                    );
                  })()}
                </Paper>
        ...
        {/* Key Actors sidebar */}
        {storyline.entities && Array.isArray(storyline.entities) && storyline.entities.length > 0 && (
      ...
      >
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                ...
                {storyline?.document_version != null && (
                  <Chip label={`Doc v${storyline.document_version}`} size='small' variant='outlined' />
                )}
                {synthesis?.created_at && (
                  <Chip label={`Generated ${new Date(synthesis.created_at).toLocaleString()}`} size='small' variant='outlined' />
                )}
              </Box>
            </Box>
        ...
        <DialogContent dividers>
          <Box sx={{
            ...
            {(synthesis?.source_articles?.length > 0 || synthesis?.sources?.length > 0) && (
              <Accordion defaultExpanded sx={{ mb: 2 }}>
                ...
                <AccordionDetails>
                  <Typography variant='caption' color='text.secondary' display='block' sx={{ mb: 1 }}>
                    Each link opens the ingested article used as input to synthesis.
                  </Typography>
                  <List dense>
                    {(synthesis?.source_articles || synthesis?.sources || []).map((row, idx) => (
                      <ListItem key={row.id ?? idx} disablePadding sx={{ py: 0.25 }}>
                        <ListItemText
                          primary={
                            row.id ? (
                              <Link component={RouterLink} to={`/${effectiveDomain}/articles/${row.id}`} underline="hover">
            ...
            <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
              Storyline editorial document version: {storyline?.document_version ?? '—'}
              {synthesis?.created_at && (
                <> · Synthesis generated at {new Date(synthesis.created_at).toLocaleString()}</>
              )}
            </Typography>
            {synthesis?.content ? (
              <Typography
