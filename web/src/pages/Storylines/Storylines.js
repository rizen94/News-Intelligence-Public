...
const Storylines = () => {
  ...
  const fetchStorylines = useCallback(async() => {
    try {
      ...
      const response = await apiService.getStorylines(params);
  ...
  const fetchCategories = useCallback(async() => {
    try {
      // Categories not currently available in API, so we'll skip this for now
      // const response = await apiService.getStorylineCategories();
      // if (response.success) {
  ...
  const fetchTimelineData = useCallback(async storylineId => {
    try {
      ...
      if (response.success) {
        // Map timeline_events to the format expected by the timeline dialog
