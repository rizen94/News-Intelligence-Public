...
function SynthesizedView() {
  ...
  async function loadSynthesis(forceRegenerate = false) {
    ...
    try {
      ...
      if (!forceRegenerate) {
        const cachedResponse = await fetch(`/api/v4/${domain}/synthesis/storyline/${id}/cached`);
  ...
  async function loadQuality() {
    try {
      const response = await fetch(`/api/v4/${domain}/synthesis/quality/${id}`);
      if (response.ok) {
        const data = await response.json();
        setQuality(data);
      }
  ...
  async function downloadMarkdown() {
    try {
      const response = await fetch(`/api/v4/${domain}/synthesis/storyline/${id}/markdown?depth=${depth}`);
      if (response.ok) {
        const data = await response.json();
