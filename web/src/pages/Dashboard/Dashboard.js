...
const Dashboard = () => {
  ...
  const fetchDashboardData = async() => {
    try {
      ...
      const [articlesRes, storylinesRes, feedsRes, healthRes] =
        await Promise.all([
      ...
      const biasAnalysis = {
        ...
        leftBias:
          articlesRes.data?.articles?.filter(a => a.bias_score < -0.3).length ||
          0,
        rightBias:
          articlesRes.data?.articles?.filter(a => a.bias_score > 0.3).length ||
          0,
        centerBias:
          articlesRes.data?.articles?.filter(a => Math.abs(a.bias_score) <= 0.3)
            .length || 0,
      };
        ...
        <Grid item xs={12} md={3}>
          <Card>
            <CardContent>
              ...
              <Box mt={1}>
                <Chip
                  label={`Left: ${dashboardData.biasAnalysis.leftBias}`}
                  size='small'
                  color='error'
                  sx={{ mr: 0.5 }}
                />
                <Chip
                  label={`Center: ${dashboardData.biasAnalysis.centerBias}`}
                  size='small'
                  color='success'
                  sx={{ mr: 0.5 }}
                />
                <Chip
                  label={`Right: ${dashboardData.biasAnalysis.rightBias}`}
                  size='small'
                  color='warning'
                />
              </Box>
                    ...
                    <ListItem key={index} divider>
                      ...
                      <ListItemText
                        ...
                        secondary={
                          <Box>
                            <Typography variant='body2' color='text.secondary'>
                              {storyline.category} • {storyline.article_count}{' '}
                              articles
                            </Typography>
                            <Box mt={0.5}>
                              <Chip
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={4}>
                  <Paper sx={{ p: 2 }}>
                    ...
                    <Typography variant='body2'>
                      Total Analyzed: {dashboardData.biasAnalysis.analyzed}
                      <br />
                      Left Bias: {dashboardData.biasAnalysis.leftBias}
                      <br />
                      Center: {dashboardData.biasAnalysis.centerBias}
                      <br />
