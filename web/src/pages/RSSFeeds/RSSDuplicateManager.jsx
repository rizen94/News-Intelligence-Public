...
const RSSDuplicateManager = () => {
  ...
  const loadDuplicateData = useCallback(async() => {
    ...
    try {
      const [duplicatesRes, similarRes, statsRes] = await Promise.all([
        api.get('/api/v4/rss-feeds/duplicates/exact'),
        api.get('/api/v4/rss-feeds/duplicates/similar'),
        api.get('/api/v4/rss-feeds/duplicates/stats'),
      ]);
  ...
  const handleDetectDuplicates = async() => {
    ...
    try {
      const response = await api.get(
        '/api/v4/rss-feeds/duplicates/detect',
      );
  ...
  const handleAutoMerge = async() => {
    ...
    try {
      const response = await api.post(
        '/api/v4/rss-feeds/duplicates/auto-merge',
        null,
        {
  ...
  const handleAddPrevention = async() => {
    ...
    try {
      const response = await api.post(
        '/api/v4/rss-feeds/duplicates/prevent',
      );
