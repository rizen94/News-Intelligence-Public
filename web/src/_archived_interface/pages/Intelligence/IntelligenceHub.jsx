...
const IntelligenceHub = () => {
  ...
  const loadMorningBriefing = useCallback(async() => {
    try {
      ...
      // Use insights as a proxy for morning briefing
      const response = await api.get(
        '/api/intelligence_hub/insights?limit=5&active_only=true',
      );
      if (response.data?.success) {
        setMorningBriefing({
          generated_at: new Date().toISOString(),
          sections: {
            system_overview: { articles_processed: 0, new_articles: 0 },
          },
        });
      } else {
  ...
  const loadTrendingTopics = useCallback(async() => {
    try {
      ...
      const response = await apiService.getTrendingTopics(24, 10);
      if (response.success) {
        const data = response.data;
        const topics = Array.isArray(data) ? data : (data?.trending_topics ?? []);
  ...
  const loadTopicClusters = useCallback(async() => {
    try {
      ...
      const response = await apiService.getBigPicture(24);
      if (response.success) {
        const dist = response.data?.topic_distribution || [];
  ...
  const runAnalysis = async(analysisType, contentId) => {
    try {
      ...
      setAnalysisError(
        `Failed to run ${analysisType} analysis: ${err.message}`,
      );
    } finally {
              ...
              {morningBriefingLoading ? (
                ...
                <Alert severity='info'>
                  <Typography variant='body2'>
                    No morning briefing data available. Click "Generate
                    Briefing" to create one.
                  </Typography>
                </Alert>
              )}
            </CardContent>
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Your personalized news summary based on your interests and
                reading patterns.
              </Typography>
                          ...
                          <Box>
                            ...
                            <Box display='flex' gap={1} alignItems='center'>
                              <Chip
                                ...
                                variant='outlined'
                              />
                              <Chip label={article.source || article.source_domain || 'Unknown'} size='small' />
                              <Typography
                                variant='caption'
                                color='text.secondary'
                              >
  ...
  const TrendsTab = () => {
    ...
    const sentimentDistribution = topics.reduce(
      (acc, topic) => {
    ...
    const positivePercent =
      total > 0
        ? Math.round((sentimentDistribution.positive / total) * 100)
        : 0;
    const negativePercent =
      total > 0
        ? Math.round((sentimentDistribution.negative / total) * 100)
        : 0;
    const neutralPercent =
      total > 0 ? Math.round((sentimentDistribution.neutral / total) * 100) : 0;
              ...
              <CardContent>
                ...
                {trendingTopicsLoading ? (
                  ...
                  <Box>
                    ...
                    <Typography variant='body2' color='text.secondary'>
                      Topics ranked by engagement and discussion volume over the
                      past 24 hours.
                    </Typography>
                  </Box>
                ) : (
                  <Alert severity='info'>
                    <Typography variant='body2'>
                      No trending topics available. Click "Refresh Trends" to
                      load data.
                    </Typography>
                  </Alert>
                )}
  ...
  const ClustersTab = () => {
    ...
    const avgQuality =
      topicClusters.length > 0
        ? topicClusters.reduce((sum, cluster) => sum + cluster.avg_quality, 0) /
          topicClusters.length
        : 0;
    const totalArticles = topicClusters.reduce(
      (sum, cluster) => sum + cluster.article_count,
      0,
    );
    const sourceDiversity = topicClusters.reduce(
      (sum, cluster) => sum + cluster.sources.length,
      0,
    );
            ...
            <Card>
              <CardContent>
                ...
                {topicClustersLoading ? (
                  ...
                  <Alert severity='info'>
                    <Typography variant='body2'>
                      No topic clusters available. Click "Refresh Clusters" to
                      load data.
                    </Typography>
                  </Alert>
                )}
            ...
            <Card>
              <CardContent>
                ...
                <Typography
                  ...
                  sx={{ mb: 2 }}
                >
                  AI-powered topic clustering helps identify related articles
                  and emerging story patterns.
                </Typography>
                {topicClustersLoading ? (
                  <Box
                    display='flex'
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            <InputLabel>
              Select {contentType === 'storyline' ? 'Storyline' : 'Article'}
            </InputLabel>
            <Select
              value={selectedContent?.id || ''}
              label={`Select ${
                contentType === 'storyline' ? 'Storyline' : 'Article'
              }`}
              onChange={e => {
                const content =
            ...
            >
              {(contentType === 'storyline' ? storylines : articles).map(
                item => (
          ...
          {selectedContent && (
            <Alert severity='info' sx={{ mb: 2 }}>
              Selected:{' '}
              <strong>
                {selectedContent.title || selectedContent.headline}
              </strong>
        ...
        <Grid item xs={12} md={4}>
          <Card>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Analyze from multiple viewpoints and sources to provide balanced
                insights.
              </Typography>
              <Button
                variant='outlined'
        ...
        <Grid item xs={12} md={4}>
          <Card>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Evaluate potential impacts across different dimensions and
                stakeholders.
              </Typography>
              <Button
                variant='outlined'
        ...
        <Grid item xs={12} md={4}>
          <Card>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Forecast future developments and trends based on current data
                patterns.
              </Typography>
              <Button
                variant='outlined'
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Integrate expert opinions and authoritative sources for
      ...
      {selectedContent && analysisResults[selectedContent.id] && (
        <Card sx={{ mt: 3 }}>
          <CardContent>
            ...
            {Object.entries(analysisResults[selectedContent.id]).map(
              ([analysisType, result]) => (
  ...
  return (
    <Box>
      ...
      {/* Context-centric (Phase 4) — quick links */}
      <Card sx={{ mb: 3, bgcolor: 'grey.50' }}>
        <CardContent>
          ...
          <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
            Entity profiles, contexts, tracked events, and pipeline status from the context-centric model.
          </Typography>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} md={3}>
              <Button
                component={Link}
                to={getDomainPath('/intelligence/entity-profiles')}
                fullWidth
                variant='outlined'
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Button
                component={Link}
                to={getDomainPath('/intelligence/contexts')}
                fullWidth
                variant='outlined'
                sx={{ py: 1.5, justifyContent: 'flex-start', textTransform: 'none' }}
                startIcon={<Article />}
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Button
                component={Link}
                to={getDomainPath('/intelligence/tracked-events')}
                fullWidth
                variant='outlined'
                sx={{ py: 1.5, justifyContent: 'flex-start', textTransform: 'none' }}
                startIcon={<EventIcon />}
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Button
                component={Link}
                to={getDomainPath('/intelligence/context-centric-status')}
                fullWidth
                variant='outlined'
                sx={{ py: 1.5, justifyContent: 'flex-start', textTransform: 'none' }}
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Button
                component={Link}
                to={getDomainPath('/intelligence/entity-management')}
                fullWidth
                variant='outlined'
                sx={{ py: 1.5, justifyContent: 'flex-start', textTransform: 'none' }}
                startIcon={<PersonIcon />}
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Button
                component={Link}
                to={getDomainPath('/intelligence/search')}
                fullWidth
                variant='outlined'
                sx={{ py: 1.5, justifyContent: 'flex-start', textTransform: 'none' }}
                startIcon={<Search />}
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Button
                component={Link}
                to={getDomainPath('/intelligence/collection-watch')}
                fullWidth
                variant='outlined'
                sx={{ py: 1.5, justifyContent: 'flex-start', textTransform: 'none' }}
      ...
      >
        ...
        <DialogContent>
          <Typography variant='body1' sx={{ mb: 2 }}>
            Run <strong>{selectedAnalysis?.replace('-', ' ')}</strong> analysis
            on:
          </Typography>
          {selectedContent && (
            <Alert severity='info'>
