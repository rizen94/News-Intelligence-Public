...
class TopicClusteringService:
    ...
    def _extract_fallback_topics(self, title: str, content: str = None) -> dict[str, Any]:
        ...
        if any(
            word in title.lower()
            for word in ["election", "vote", "campaign", "president", "senate", "congress"]
        ):
            category = "Politics"
        elif any(
            word in title.lower() for word in ["climate", "environment", "carbon", "emissions"]
        ):
            category = "Climate"
        elif any(word in title.lower() for word in ["tech", "ai", "software", "digital", "cyber"]):
            category = "Technology"
        elif any(
            word in title.lower() for word in ["economy", "market", "stock", "inflation", "gdp"]
        ):
            category = "Economy"
    ...
    def _fallback_clustering(self, articles: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            "success": True,
            "data": {"topics": topics, "unclustered_articles": [], "total_topics": len(topics)},
            "timestamp": datetime.now().isoformat(),
        }

    async def save_topics_to_database(
        self, topics_data: dict[str, Any], articles: list[dict[str, Any]]
    ) -> bool:
