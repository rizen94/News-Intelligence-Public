                ...
                if schema_exists:
                    ...
                    if table_count >= 3:
                        ...
                        print_result(
                            ...
                            False,
                            f"Only {table_count} tables found (expected 3+)",
                        )
...
def test_domain_data(domain: str) -> dict[str, Any]:
    ...
    schema_name = domain.replace("-", "_")
    results = {"articles": 0, "storylines": 0, "rss_feeds": 0}
...
def test_all_domain_data() -> dict[str, dict[str, int]]:
    ...
    for domain in DOMAINS:
        ...
        print_result(f"{domain} - Articles", articles > 0, f"{articles} articles found")
        print_result(f"{domain} - Storylines", storylines > 0, f"{storylines} storylines found")
        print_result(f"{domain} - RSS Feeds", feeds > 0, f"{feeds} RSS feeds found")

    return all_results


def test_api_endpoint(
    method: str, url: str, params: dict = None, data: dict = None
) -> dict[str, Any]:
...
def test_api_endpoints() -> dict[str, Any]:
    ...
    for domain in DOMAINS:
        ...
        articles_result = test_api_endpoint(
            "GET", f"{API_BASE_URL}/api/{domain}/articles", params={"limit": 10, "offset": 0}
        )
        print_result(
            f"{domain} - Articles",
        ...
        storylines_result = test_api_endpoint(
            "GET", f"{API_BASE_URL}/api/{domain}/storylines", params={"limit": 10, "offset": 0}
        )
        print_result(
            f"{domain} - Storylines",
        ...
        rss_result = test_api_endpoint("GET", f"{API_BASE_URL}/api/{domain}/rss_feeds")
        print_result(
            f"{domain} - RSS Feeds",
            rss_result["success"],
...
def test_pagination() -> dict[str, Any]:
    ...
    page1_result = test_api_endpoint(
        "GET", f"{API_BASE_URL}/api/{domain}/articles", params={"limit": 5, "offset": 0}
    )
        ...
        if isinstance(data, dict):
            ...
            if total > 5:
                page2_result = test_api_endpoint(
                    "GET", f"{API_BASE_URL}/api/{domain}/articles", params={"limit": 5, "offset": 5}
                )
