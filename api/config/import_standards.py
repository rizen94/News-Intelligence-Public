...
STANDARD_IMPORTS = {
    ...
    "local": ["config", "services", "routes", "schemas", "middleware", "modules", "utils"],
}
    ...
    try:
            ...
            for pattern in IMPORT_RULES["forbidden_patterns"]:
                if pattern in import_line:
                    results["errors"].append(
                        f"Line {line_num}: Forbidden pattern '{pattern}' in '{import_line}'"
                    )
                    results["valid"] = False
        ...
        for line_num, import_line in import_lines:
            if import_line.startswith("from ."):
                results["warnings"].append(
                    f"Line {line_num}: Relative import '{import_line}' - consider using absolute import"
                )
        ...
        if third_party_imports and local_imports:
            ...
            if first_local < last_third_party:
                results["warnings"].append(
                    "Import order issue: local imports should come after third-party imports"
                )
    ...
    try:
        ...
        for match in matches:
            ...
            results["changes"].append(
                f"Fixed double relative import: {old_pattern} -> {new_import}"
            )
...
def validate_all_imports(project_root: str = None) -> dict[str, Any]:
    ...
    # Create summary
    results["summary"] = {
        "validation_rate": results["valid_files"] / results["total_files"]
