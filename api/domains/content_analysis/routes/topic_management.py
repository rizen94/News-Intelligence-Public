...
async def submit_feedback(assignment_id: int, feedback: TopicFeedback):
    ...
    try:
        ...
        if not result.get('success'):
            raise HTTPException(status_code=500, detail=result.get('error', 'Feedback recording failed'))
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                existing_topics = {row[0]: {'name': row[1], 'status': row[2]} for row in cur.fetchall()}
                ...
                merged_topics = [tid for tid, data in existing_topics.items() if data['status'] == 'merged']
                if merged_topics:
                    raise HTTPException(
                        status_code=400, 
                ...
                primary_category = primary_row[2]
                ...
                new_confidence = float(score_row[0] or primary_confidence) if score_row[0] else primary_confidence
                ...
                for row in assignments_to_transfer:
                    ...
                    if article_id not in assignments_by_article:
                        ...
                        if float(row[1] or 0.5) > assignments_by_article[article_id]['confidence_score']:
                            assignments_by_article[article_id] = {
                                'article_id': row[0],
                                'confidence_score': float(row[1] or 0.5),
                                'relevance_score': float(row[2] or 0.5),
                ...
                logger.info(f"Merged {len(topics_to_merge)} topics into {primary_name}: {merged_names}")
                
                return {
                    "success": True,
                    "data": {
                        "primary_topic": {
                            "id": updated_primary[0],
                            "name": updated_primary[1]
                        },
                        "merged_topics": [
