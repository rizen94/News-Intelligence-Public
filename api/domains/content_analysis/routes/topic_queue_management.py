...
router = APIRouter(
    prefix="/api", tags=["Topic Extraction Queue"], responses={404: {"description": "Not found"}}
)
...
async def start_queue_worker(
    ...
    try:
        ...
        if schema in queue_workers and queue_workers[schema].is_running:
            return {"success": True, "message": "Queue worker already running", "domain": domain}
...
async def process_queue_manually(
    domain: str = Path(..., pattern=DOMAIN_PATH_PATTERN), batch_size: int = Query(10, ge=1, le=50)
):
