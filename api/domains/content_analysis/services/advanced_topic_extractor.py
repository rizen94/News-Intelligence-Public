...
class AdvancedTopicExtractor:
    ...
    def __init__(self, db_connection_func, schema: str = "politics"):
        ...
        self.stop_words = {
            ...
            'think', 'thought', 'take', 'took', 'make', 'made', 'give', 'gave', 'use', 'used', 'find', 'found',
            'tell', 'told', 'ask', 'asked', 'work', 'worked', 'try', 'tried', 'call', 'called', 'need', 'needed',
            'want', 'wanted', 'seem', 'seemed', 'feel', 'felt', 'become', 'became', 'leave', 'left', 'put',
            'set', 'run', 'ran', 'move', 'moved', 'live', 'lived', 'believe', 'believed', 'hold', 'held',
            # More action verbs
            'complete', 'finish', 'finished', 'start', 'started', 'begin', 'began', 'end', 'ended',
            'reach', 'reached', 'reach', 'chase', 'chased', 'catch', 'caught', 'win', 'won', 'lose', 'lost',
            'beat', 'beaten', 'defeat', 'defeated', 'play', 'played', 'score', 'scored', 'scoring',
            'break', 'broke', 'broken', 'hit', 'hitting', 'strike', 'struck', 'throw', 'threw', 'thrown',
            'run', 'runs', 'running', 'walk', 'walked', 'walking', 'jump', 'jumped', 'jumping',
            # Present participles (verb forms ending in -ing)
            'becoming', 'getting', 'going', 'coming', 'seeing', 'knowing', 'thinking', 'taking', 'making',
            'giving', 'using', 'finding', 'telling', 'asking', 'working', 'trying', 'calling', 'needing',
            'wanting', 'seeming', 'feeling', 'becoming', 'leaving', 'putting', 'setting', 'running',
            'moving', 'living', 'believing', 'holding', 'finishing', 'starting', 'beginning', 'ending',
            'reaching', 'chasing', 'catching', 'winning', 'losing', 'beating', 'defeating', 'playing',
            'scoring', 'breaking', 'hitting', 'striking', 'throwing', 'walking', 'jumping',
            # Common news/reporting verbs
            'report', 'reported', 'reporting', 'announce', 'announced', 'announcing', 'confirm', 'confirmed',
            'reveal', 'revealed', 'revealing', 'claim', 'claimed', 'claiming', 'state', 'stated', 'stating',
            'declare', 'declared', 'declaring', 'note', 'noted', 'noting', 'add', 'added', 'adding',
            'explain', 'explained', 'explaining', 'describe', 'described', 'describing', 'show', 'showed',
            'showing', 'indicate', 'indicated', 'indicating', 'suggest', 'suggested', 'suggesting',
            # Verbs that don't make good topics
            'happen', 'happened', 'happening', 'occur', 'occurred', 'occurring', 'continue', 'continued',
            'continuing', 'begin', 'began', 'beginning', 'start', 'started', 'starting',
            # Common adjectives/adverbs
            'new', 'old', 'good', 'bad', 'long', 'short', 'big', 'small', 'high', 'low', 'early', 'late',
            'right', 'wrong', 'first', 'last', 'next', 'previous', 'only', 'also', 'very', 'much', 'many',
            'more', 'most', 'some', 'any', 'all', 'each', 'every', 'both', 'half', 'full', 'empty',
            # Action/state adjectives that aren't meaningful topics
            'complete', 'finished', 'unfinished', 'started', 'beginning', 'ending', 'ongoing', 'active',
            'inactive', 'open', 'closed', 'public', 'private', 'free', 'available', 'unavailable',
            # Sports/action descriptors
            'unbeaten', 'defeated', 'winning', 'losing', 'leading', 'trailing', 'ahead', 'behind',
            'final', 'semi', 'quarter', 'round', 'match', 'game', 'match', 'matchup',
            ...
            'cup', 'championship', 'tournament', 'league', 'series', 'season', 'playoff', 'final',
            'semi', 'quarter', 'round', 'match', 'matchup', 'game', 'play', 'player', 'players',
            'fan', 'fans', 'coach', 'team', 'teams', 'score', 'scores', 'point', 'points',
            # News/article specific words
            'article', 'articles', 'news', 'report', 'reports', 'story', 'stories', 'update', 'updates',
            'latest', 'breaking', 'according', 'source', 'sources', 'official', 'officials',
            # Common connectors and filler words
            'one', 'two', 'three', 'four', 'five', 'first', 'second', 'third', 'when', 'where', 'why',
            'how', 'what', 'who', 'which', 'there', 'here', 'just', 'now', 'then', 'than', 'well',
            # Additional common words
            'way', 'day', 'may', 'way', 'up', 'out', 'so', 'if', 'about', 'into', 'through', 'during',
            'including', 'against', 'among', 'throughout', 'despite', 'towards', 'upon', 'concerning',
            'like', 'such', 'same', 'other', 'another', 'own', 'its', 'our', 'your', 'their', 'his',
            'her', 'my', 'yourself', 'itself', 'themselves', 'ourselves', 'himself', 'herself',
            # Common phrases parts that aren't meaningful alone
            'from', 'during', 'before', 'after', 'above', 'below', 'under', 'over', 'across', 'around',
            'inside', 'outside', 'within', 'without', 'through', 'between', 'among', 'toward', 'towards'
        }
        
        # Additional filter: common meaningless single words (less than 4 chars, very common)
        self.uncommon_short_words = {
            'fly', 'run', 'set', 'try', 'win', 'cut', 'hit', 'put', 'let', 'yet', 'nor', 'ago', 'per',
            'via', 'etc', 'etc.', 'vs', 'vs.', 'ie', 'ie.', 'eg', 'eg.', 'mr', 'mrs', 'dr', 'pm', 'am',
            'uk', 'us', 'tv', 'ai', 'id', 'ad', 'bc', 'ce', 'qa', 'hr', 'it', 'it\'s', 'is\'nt', 'don\'t',
            'won\'t', 'can\'t', 'hasn\'t', 'haven\'t', 'didn\'t', 'wouldn\'t', 'couldn\'t', 'shouldn\'t',
            # Common action words that are too generic
            'cup', 'final', 'semi', 'match', 'game', 'play', 'score', 'point', 'round', 'team'
        }
        
        # Words that indicate a phrase is likely not a meaningful topic (too verb-heavy)
        self.verb_indicators = {
            'becoming', 'reaching', 'chasing', 'completing', 'finishing', 'starting', 'ending',
            'winning', 'losing', 'playing', 'scoring', 'beating', 'defeating', 'running',
            'breaking', 'hitting', 'striking', 'throwing', 'catching', 'jumping', 'walking',
            'reporting', 'announcing', 'confirming', 'revealing', 'claiming', 'stating',
            'declaring', 'explaining', 'describing', 'showing', 'indicating', 'suggesting',
            'happening', 'occurring', 'continuing', 'beginning', 'starting', 'finishing'
        }
        
        # Well-known compound phrases that should be allowed even if they contain stop words
        self.allowed_compound_phrases = {
            'world cup', 'world series', 'super bowl', 'olympic games', 'olympics',
            'united states', 'united kingdom', 'new york', 'los angeles', 'san francisco',
            'prime minister', 'president', 'white house', 'supreme court', 'house representatives'
        }
        
        # Web-related terms that should never be topics (unless tech-focused)
        self.web_artifacts = {
            'href', 'http', 'https', 'www', 'url', 'html', 'htm', 'css', 'js', 'javascript',
            'xml', 'json', 'api', 'www', 'com', 'org', 'net', 'edu', 'gov', 'link', 'links',
            'anchor', 'tag', 'tags', 'attribute', 'attributes', 'element', 'elements',
            'domain', 'domains', 'website', 'websites', 'webpage', 'webpages', 'page', 'pages',
            'click', 'view', 'read', 'more', 'here', 'there', 'source', 'sources'
        }
    ...
    def _clean_text_for_topic_extraction(self, text: str) -> str:
        ...
        text = re.sub(r'[a-zA-Z0-9-]+\.(com|org|net|edu|gov|io|co|uk|us|de|fr|ca|au|jp|cn)[^\s]*', ' ', text, flags=re.IGNORECASE)
    ...
    def _extract_phrase_topics(self, articles: List[Tuple]) -> List[TopicInsight]:
        ...
        for article_id, title, content, summary, published_at, sentiment_score, source_domain in articles:
            ...
            for i in range(len(filtered_words) - 2):
                word1, word2, word3 = filtered_words[i], filtered_words[i+1], filtered_words[i+2]
                # At least 2 of 3 words should be meaningful (4+ chars)
                meaningful_count = sum(1 for w in [word1, word2, word3] 
                ...
                if meaningful_count >= 2:
                    ...
                    if len(phrase) > 12 and not any(word in self.stop_words for word in [word1, word2, word3]):
                        phrase_freq[phrase] += 1
                        article_phrases[phrase].append(article_id)
            ...
            for i in range(len(filtered_words) - 3):
                ...
                meaningful_count = sum(1 for w in [word1, word2, word3, word4] 
                                     if len(w) >= 4 or w in self._get_category_all_keywords())
                if meaningful_count >= 3:
                    # Skip if too many verbs (not a good topic phrase)
                    verb_count = sum(1 for w in [word1, word2, word3, word4] if w in self.verb_indicators)
                    if verb_count >= 2:  # If 2+ verbs in a 4-word phrase, likely not a good topic
                        continue
                    phrase = f"{word1} {word2} {word3} {word4}"
                    if len(phrase) > 18 and not any(word in self.stop_words for word in [word1, word2, word3, word4]):
                        phrase_freq[phrase] += 1
                        article_phrases[phrase].append(article_id)
    ...
    def _is_too_generic_phrase(self, phrase: str) -> bool:
        ...
        generic_word_count = sum(1 for word in words if word in self.stop_words or word in self.uncommon_short_words)
        ...
        if len(words) >= 3:
            ...
            all_verbs = all(word in self.verb_indicators or word in self.stop_words for word in words)
            if all_verbs:
                return True
    ...
    def _extract_entity_topics(self, articles: List[Tuple]) -> List[TopicInsight]:
        ...
        non_entity_words = {
            ...
            'With', 'By', 'From', 'About', 'This', 'That', 'These', 'Those',
            'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'
        }
        ...
        for article_id, title, content, summary, published_at, sentiment_score, source_domain in articles:
            ...
            for entity in entities + multi_word_entities:
                ...
                if any(entity_lower.startswith(prefix) for prefix in ['http', 'www', 'href', 'url']):
                    continue
    ...
    def _merge_similar_topics(self, topics: List[TopicInsight]) -> List[TopicInsight]:
        ...
        sorted_topics = sorted(topics, key=lambda t: (len(t.name.split()), t.frequency), reverse=True)
    ...
    def _are_topics_similar(self, topic1: TopicInsight, topic2: TopicInsight) -> bool:
        ...
        if name1_lower in name2_lower or name2_lower in name1_lower:
            ...
            if len(name1_lower) >= 4 and len(name2_lower) >= 4:
                ...
                if re.search(r'\b' + re.escape(name1_lower) + r'\b', name2_lower) or \
                   re.search(r'\b' + re.escape(name2_lower) + r'\b', name1_lower):
                    return True
    ...
    def _merge_topic_list(self, topics: List[TopicInsight]) -> TopicInsight:
        ...
        base_topic = max(topics, key=lambda t: (len(t.name.split()), t.frequency, t.relevance_score))
