...
class TopicIntelligenceService:
    ...
    def filter_intelligent_topics(self, topics: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        filtered_topics.sort(
            key=lambda x: (x.get("quality_score", 0), x.get("avg_confidence", 0)), reverse=True
        )
    ...
    def get_intelligent_word_cloud(self, limit: int = 30) -> dict[str, Any]:
        ...
        try:
                    ...
                    topics.append(
                        {
                            ...
                            "latest_article_date": row["latest_article_date"].isoformat()
                            if row["latest_article_date"]
                            else None,
                        }
                    )
            ...
            return {"success": False, "error": str(e), "data": {"word_cloud": []}}
        finally:
            if "conn" in locals():
                conn.close()
