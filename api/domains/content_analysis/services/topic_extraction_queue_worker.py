        ...
        try:
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    if reset_count:
                        logger.info(
                            "♻️ Reset %d stale 'processing' records to 'pending' for %s",
                            reset_count,
                            self.schema,
                        )
            finally:
    ...
    async def _process_queue_batch(self) -> bool:
        ...
        try:
            ...
            if should_yield_to_api():
                logger.debug(
                    "Yielding to API — skipping topic extraction this cycle for %s", self.schema
                )
                return False
        ...
        try:
            with conn.cursor() as cur:
                ...
                if not queued_items:
                    ...
                    if unprocessed:
                        logger.info(
                            "📋 Found %d unprocessed articles, queueing them…", len(unprocessed)
                        )
