...
async def get_intelligence_insights(
    insight_type: str | None = None, limit: int = 20, active_only: bool = True
):
...
async def get_trend_predictions(
    prediction_type: str | None = None, limit: int = 20, active_only: bool = True
):
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                analytics["avg_article_quality"] = round((weighted_aq / aq_n) if aq_n else 0, 2)
                analytics["avg_storyline_quality"] = round((weighted_sq / sq_n) if sq_n else 0, 2)

                return {"success": True, "data": analytics, "timestamp": datetime.now().isoformat()}
...
async def process_intelligence_analysis(articles: list[tuple]):
    ...
    try:
        ...
        insights_result = await llm_service.generate_summary(
            analysis_context, TaskType.COMPREHENSIVE_ANALYSIS
        )
                    ...
                    with conn.cursor() as cur:
                        ...
                        """,
                            (
                                ...
                                {
                                    "articles_analyzed": len(articles),
                                    "model_used": insights_result["model_used"],
                                },
                                85,  # High confidence
                                90,  # High relevance
                                datetime.now(),
                        ...
                        conn.commit()
                        logger.info(
                            f"Generated intelligence insights from {len(articles)} articles"
                        )
                finally:
                    conn.close()
...
async def process_trend_prediction(articles: list[tuple], timeframe: str, category: str):
    ...
    try:
        ...
        prediction_result = await llm_service.generate_summary(
            trend_context, TaskType.COMPREHENSIVE_ANALYSIS
        )
            ...
            if conn:
                try:
                    with conn.cursor() as cur:
                        ...
                        conn.commit()
                        logger.info(
                            f"Generated trend prediction for {category} from {len(articles)} articles"
                        )
                finally:
                    conn.close()
