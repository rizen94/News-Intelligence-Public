...
def post_claim_feedback(
    ...
    validated_by: str | None = Body(None, embed=True),
) -> dict[str, Any]:
    ...
    return {
        "success": True,
        "data": {"validation_id": result.get("validation_id")},
        "message": None,
    }
...
def post_event_validation(
    ...
    validation_status: str = Body(..., embed=True),
    corrections: dict[str, Any] | None = Body(None, embed=True),
) -> dict[str, Any]:
...
def get_quality_extraction_metrics(
    ...
    phase: str | None = Query(None, description="Filter by phase (e.g. claim_extraction)"),
    since: int | None = Query(None, description="Only metrics since N days ago"),
) -> dict[str, Any]:
...
def get_quality_source_rankings(
    domain: str | None = Query(
        None, description="Filter by domain (substring match on source_name)"
    ),
