...
async def get_route_supervisor_health():
    ...
    try:
        ...
        return {
            ...
            "last_check": supervisor.last_full_check.isoformat()
            if supervisor.last_full_check
            else None,
        }
    ...
    try:
        ...
        return {
            ...
            "frontend_health": {
                ...
                "status": report.frontend_health.status.value
                if report.frontend_health
                else "unknown",
                "response_time_ms": report.frontend_health.response_time_ms
                if report.frontend_health
                else None,
                "error": report.frontend_health.error_message if report.frontend_health else None,
                "api_connection": report.frontend_health.api_connection
                if report.frontend_health
                else False,
                "consecutive_failures": report.frontend_health.consecutive_failures
                if report.frontend_health
                else 0,
                "last_check": report.frontend_health.last_check.isoformat()
                if report.frontend_health
                else None,
            }
            if report.frontend_health
...
async def get_recent_issues(hours: int = 24, limit: int = 100):
    ...
    try:
        ...
        return {"success": True, "count": len(issues), "hours": hours, "issues": issues[:limit]}
    except Exception as e:
