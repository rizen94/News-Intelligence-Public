...
class QualityAssessmentService(DomainAwareService):
    ...
    async def _calculate_quality_metrics(
        self, storyline: dict, articles: list[dict]
    ) -> dict[str, Any]:
        ...
        quality_score = (
            avg_quality * 0.4 + avg_credibility * 0.3 + diversity_score * 0.2 + completeness * 0.1
        )
    ...
    async def _assess_narrative_coherence(
        ...
        # Overall coherence
        coherence_score = (
            temporal_coherence * 0.3 + thematic_coherence * 0.4 + source_coherence * 0.3
        )
    ...
    async def _assess_professional_standards(
        ...
        standards_score = (
            count_score * 0.3 + diversity_score * 0.3 + summary_score * 0.2 + avg_quality * 0.2
        )
    ...
    def _generate_recommendations(
        self, overall_score: float, coherence: dict, accuracy: dict, standards: dict
    ) -> list[str]:
        ...
        if overall_score < 0.7:
            recommendations.append(
                "Overall quality is below professional standards. Consider adding more high-quality sources."
            )

        if coherence["coherence_score"] < 0.7:
            recommendations.append(
                "Narrative coherence could be improved. Ensure articles are thematically related and chronologically ordered."
            )

        if accuracy["accuracy_score"] < 0.8:
            recommendations.append(
                "Factual accuracy could be improved. Add more cross-source verification."
            )

        if standards["standards_score"] < 0.7:
            recommendations.append(
                "Professional standards could be improved. Add more diverse sources and comprehensive analysis."
            )

        if coherence["source_coherence"] < 0.5:
            recommendations.append(
                "Source diversity is low. Add articles from different sources to improve credibility."
            )
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    return {
                        ...
                        "data": {
                            ...
                            "cross_source_verification": accuracy_assessment[
                                "cross_source_verification"
                            ],
                            "source_count": accuracy_assessment["source_count"],
                            "validation_status": "verified"
                            if accuracy_assessment["accuracy_score"] >= 0.8
                            else "needs_review",
                        },
                    }
    ...
    async def suggest_improvements(self, storyline_id: int) -> dict[str, Any]:
        ...
        return {
            ...
            "data": {
                ...
                "priority": "high"
                if data.get("overall_quality_score", 0.0) < 0.6
                else "medium"
                if data.get("overall_quality_score", 0.0) < 0.8
                else "low",
            },
        }
