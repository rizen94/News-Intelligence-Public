...
class StorylineService(DomainAwareService):
    ...
    async def evolve_storyline_with_new_content(
        self,
        storyline_id: int,
        new_article_ids: Optional[List[int]] = None,
        ...
        try:
            ...
            try:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    existing_summary = storyline.get('analysis_summary') or storyline.get('description') or ""
                    existing_context = {
                        "key_facts": [],
                        "entities": [],
                        "dates": [],
                    ...
                    if not updated_summary and articles:
                        logger.info(f"Generating initial summary for storyline {storyline_id} from {len(articles)} articles")
                        initial_summary_result = await self._generate_initial_summary(articles, storyline)
                        if initial_summary_result.get("success"):
                            updated_summary = initial_summary_result.get("data", {}).get("summary", "")
                            # Extract context from all articles for initial setup
                            for article in articles[:10]:  # Process first 10 for initial context
                                try:
                                    extraction_result = await content_extractor.extract_article_information(article)
                                    if extraction_result.get("success"):
                                        extracted_info = extraction_result.get("data", {})
                                        updated_context['key_facts'].extend(extracted_info.get('key_facts', [])[:3])
                                        updated_context['entities'].extend(extracted_info.get('entities', [])[:5])
                                        updated_context['dates'].extend(extracted_info.get('dates', [])[:3])
                                except Exception as e:
                                    logger.warning(f"Error extracting initial context from article {article.get('id')}: {e}")
                    
                    # Process new articles: extract information and merge context
                    for new_article in new_articles_data:
                        try:
                            # Extract information from new article
                            extraction_result = await content_extractor.extract_article_information(new_article)
                            if extraction_result.get("success"):
                                extracted_info = extraction_result.get("data", {})
                                ...
                                if new_info_result.get("success") and new_info_result.get("data", {}).get("has_new_information"):
                                    ...
                                    if merge_result.get("success"):
                                        ...
                                        updated_summary = merge_data.get("updated_summary", updated_summary)
                                        updated_context = merge_data.get("updated_context", updated_context)
                            ...
                            logger.warning(f"Error processing article {new_article.get('id')} for evolution: {e}")
                            continue
...
"""
            ...
            if llm_result.get("success"):
                ...
                summary += "\n\n".join([f"- {a.get('title', '')}: {a.get('summary', '')[:200]}" for a in articles[:10]])
                return {
                    "success": True,
                    "data": {
                        "summary": summary,
    ...
    async def _update_quality_metrics(
        ...
        try:
            ...
            quality_scores = [float(a.get('quality_score', 0.5)) for a in articles if a.get('quality_score')]
            ...
            if articles:
                ...
                if dates:
                    time_span = (max(dates) - min(dates)).days if len(dates) > 1 else 0
                    completeness = min(1.0, article_count / 10.0)  # 10 articles = complete
                else:
                    completeness = min(1.0, article_count / 10.0)
            else:
                completeness = 0.0
        ...
        try:
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    return {
                        "success": True,
                        "message": "Storyline archived successfully"
                    }
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    return {
                        ...
                        "data": {
                            ...
                            "last_evolution_at": row['last_evolution_at'].isoformat() if row.get('last_evolution_at') else None,
        ...
        try:
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    return {
                        "success": True,
                        "message": f"Storyline '{title}' deleted successfully"
                    }
...
"""
            ...
            if llm_result.get("success"):
                ...
                master_summary = f"Storyline: {storyline.get('title', 'Untitled')}\n\n{formatted_summary}"
            else:
                # Fallback summary
                master_summary = self._create_fallback_summary(
                    storyline.get('title', 'Untitled'),
                    articles_context
                )
