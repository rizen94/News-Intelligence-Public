...
"""
    ...
    def _basic_extraction(self, text: str) -> Dict[str, Any]:
        ...
        key_sentences = [s.strip() for s in sentences if len(s.strip()) > 50 and any(
            keyword in s.lower() for keyword in ['announced', 'reported', 'confirmed', 'revealed', 'according']
        )][:10]
    ...
    def _parse_extraction(self, llm_text: str, original_text: str) -> Dict[str, Any]:
        ...
        if "Key Facts:" in llm_text or "1." in llm_text:
            ...
            facts = [f.strip() for f in facts_section.split('\n') if f.strip() and len(f.strip()) > 20][:10]
        
        if "Entities:" in llm_text or "2." in llm_text:
            entities_section = self._extract_section(llm_text, ["Entities:", "2.", "People/Organizations:"])
        ...
        try:
            ...
            for entity in extracted_info.get('entities', []):
                ...
                for existing_entity in existing_entities:
                    if entity_lower == existing_entity.lower() or entity_lower in existing_entity.lower():
                        is_new = False
                        break
                if is_new:
            ...
            return {
                ...
                "data": {
                    ...
                    "has_new_information": len(new_facts) > 0 or len(new_entities) > 0 or len(new_dates) > 0
                }
            }
...
"""
            ...
            # Remove duplicates
            updated_context['key_facts'] = list(dict.fromkeys(updated_context['key_facts']))  # Preserve order
