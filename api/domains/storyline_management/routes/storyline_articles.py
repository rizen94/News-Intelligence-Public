...
from ..schemas.storyline_schemas import AddArticleRequest, ArticleSummary, PaginationInfo
from ..routes.storyline_management import trigger_storyline_evolution

logger = logging.getLogger(__name__)

router = APIRouter(
    tags=["Storyline Articles"],
...
async def add_article_to_domain_storyline(
    ...
    request: Optional[AddArticleRequest] = None,
...
async def get_domain_available_articles_for_storyline(
    ...
    page_size: int = Query(50, ge=1, le=100, description="Items per page (max 100 for performance)"),
    search: Optional[str] = Query(None, description="Search term for article title/summary")
):
