...
def _format_suggestion_row(row, include_storyline: bool = False) -> dict[str, Any]:
    ...
    if include_storyline:
        return {
            "suggestion_id": row[0],
            "storyline_id": row[1],
            "storyline_title": row[2],
            "article": {
                "id": row[3],
                "title": row[14],
                "summary": row[15],
                "url": row[16],
                "source_domain": row[17],
                "published_at": row[18].isoformat() if row[18] else None,
            },
            "scores": {
                "relevance": float(row[4]) if row[4] else 0.0,
                "semantic": float(row[5]) if row[5] else 0.0,
                "keyword": float(row[6]) if row[6] else 0.0,
                "quality": float(row[7]) if row[7] else 0.0,
                "combined": float(row[8]) if row[8] else 0.0,
            },
            "matched_keywords": row[9] or [],
            "matched_entities": row[10] or [],
            "reasoning": row[11],
            "suggested_at": row[12].isoformat() if row[12] else None,
...
_SUGGESTION_QUEUE_SELECT = """
    SELECT s.id, s.storyline_id, sl.title AS storyline_title,
           s.article_id, s.relevance_score, s.semantic_score,
           s.keyword_score, s.quality_score, s.combined_score,
           s.matched_keywords, s.matched_entities, s.reasoning,
           s.suggested_at, s.expires_at,
           a.title, a.summary, a.url, a.source_domain, a.published_at
...
async def get_review_queue_count(
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT COUNT(*) FROM public.storyline_article_suggestions
                WHERE domain_key = %s AND status = 'pending'
                """,
                (domain,),
            )
            count = cur.fetchone()[0]
        return {"success": True, "data": {"count": count}, "domain": domain}
    finally:
        conn.close()
    ...
    try:
        with conn.cursor() as cur:
            ...
            if storyline_id is not None:
                where += " AND s.storyline_id = %s"
                params.append(storyline_id)

            cur.execute(
                f"""
                SELECT COUNT(*) FROM public.storyline_article_suggestions s
                {where}
                """,
                params,
            )
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE id = %s AND domain_key = %s AND storyline_id = %s AND status = 'pending'
                """, (datetime.now(), reason, suggestion_id, domain, storyline_id))
