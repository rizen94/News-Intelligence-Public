...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("logs/article_deduplication.log")],
)
...
class ArticleDeduplicationSystem:
            ...
            if parsed.query:
                ...
                for param in parsed.query.split("&"):
                    if param and not any(
                        track in param.lower() for track in ["utm_", "fbclid", "gclid", "ref"]
                    ):
    ...
    def detect_content_similarities(self) -> list[dict[str, Any]]:
        ...
        with self.conn.cursor() as cur:
            ...
            for i, (id1, title1, content1, url1, domain1, created1) in enumerate(articles):
                ...
                for j, (id2, title2, content2, url2, domain2, created2) in enumerate(
                    articles[i + 1 :], i + 1
                ):
                    if id2 in processed:
                        continue
    ...
    def generate_deduplication_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "total_issues": len(url_duplicates)
                + len(normalized_url_duplicates)
                + len(content_duplicates)
                + len(title_similarities)
                + len(content_similarities),
            },
    ...
    def merge_duplicates(
        self, duplicates: list[dict[str, Any]], dry_run: bool = True
    ) -> dict[str, Any]:
        ...
        for duplicate in duplicates:
            try:
                if duplicate["type"] == "exact_url":
                    ...
                    logger.info(
                        f"{'DRY RUN: ' if dry_run else ''}Merged {len(remove_articles)} duplicates for {duplicate['url']}"
                    )
        ...
        if report["url_duplicates"]:
            ...
            if response.lower() == "y":
                ...
                merge_results = deduplicator.merge_duplicates(
                    report["url_duplicates"], dry_run=False
                )
