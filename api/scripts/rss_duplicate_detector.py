...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("logs/rss_duplicate_detector.log")],
)
...
class RSSDuplicateDetector:
    ...
    def detect_similar_feeds(self) -> list[dict[str, Any]]:
            ...
            for sch in DOMAIN_DATA_SCHEMAS:
                ...
                for feed_id, name, url, is_active in feeds:
                    try:
                        ...
                        logger.warning(f"⚠️ Could not parse URL {url!r} in {sch}: {e}")
        ...
        logger.info(f"📊 Found {len(similar_groups)} domain groups with multiple feeds")
    ...
    def detect_name_similarities(self) -> list[dict[str, Any]]:
        ...
        with self.conn.cursor() as cur:
            for sch in DOMAIN_DATA_SCHEMAS:
                ...
                for feed_id, name, url, is_active in feeds:
                    normalized = (
                        (name or "").lower().replace(" ", "").replace("-", "").replace("_", "")
                    )
    ...
    def auto_merge_duplicates(
        ...
        for duplicate in duplicates:
            ...
            try:
                ...
                logger.info(
                    f"{'DRY RUN: ' if dry_run else ''}Merged {len(remove_feed_ids)} duplicates for {duplicate['url']}"
                )
        ...
        if report["exact_duplicates"]:
            ...
            if response.lower() == "y":
                ...
                merge_results = detector.auto_merge_duplicates(
                    report["exact_duplicates"], dry_run=False
                )
