...
from api.config.paths import MIGRATIONS_DIR
...
class IntelligenceDatabaseManager:
    ...
    def mark_articles_for_processing(
        self, article_ids: list[int] = None, status: str = "processing"
    ) -> bool:
...
Examples:
  ...
  python manage_intelligence_database.py cleanup-recommendations
    ...
    rag_parser.add_argument(
        "--context-needed", action="store_true", default=True, help="Mark as needing context"
    )
    ...
    cleanup_parser.add_argument(
        "--max-age", type=int, default=30, help="Maximum age in days for cleanup"
    )
    ...
    try:
        ...
        if args.command == "update-schema":
            ...
            print(
                f"  Schema up to date: {'✅' if schema_status.get('schema_up_to_date') else '❌'}"
            )
            ...
            success = manager.mark_articles_for_rag(
                article_ids, args.context_needed, args.keep_longer, args.priority
            )
            ...
            if result.get("success"):
                if args.dry_run:
                    print(
                        f"✅ Dry run completed. Found {len(result.get('eligible_for_cleanup', []))} articles eligible for cleanup"
                    )
                else:
                    print(
                        f"✅ Cleanup completed. Removed {result.get('articles_removed', 0)} articles"
                    )
            else:
