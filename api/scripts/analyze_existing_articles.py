...
def analyze_domain_articles(
    conn, schema_name: str, limit: int | None = None, source_filter: str | None = None
...
def print_summary(
    all_stats: dict[str, dict], all_filtered: list[dict], source_filter: str | None = None
):
    ...
    print(
        f"   Articles that would be filtered: {total_filtered:,} ({total_filtered / total_articles * 100:.1f}%)"
    )
    print(
        f"   Articles that would pass: {total_articles - total_filtered:,} ({(total_articles - total_filtered) / total_articles * 100:.1f}%)"
    )
        ...
        if stats["total"] > 0:
            ...
            if stats["filtered"] > 0:
                ...
                if stats.get("excluded_content", 0) > 0:
                    print(
                        f"         - Excluded content (sports/entertainment): {stats['excluded_content']:,}"
                    )
    ...
    if all_filtered:
        ...
        for i, article in enumerate(all_filtered[:20], 1):
            ...
            print(
                f"      Scores: Quality={article['quality_score']:.2f}, Impact={article['impact_score']:.2f}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze existing articles against filtering criteria"
    )
    parser.add_argument("--source", type=str, help='Filter by source name (e.g., "telegraph")')
    parser.add_argument(
        "--limit", type=int, help="Limit articles analyzed per domain (default: all)"
    )
    parser.add_argument("--domains", nargs="+", help="Specific domains to analyze (default: all)")
    parser.add_argument(
        "--export", type=str, help="Export filtered article IDs to file (CSV format)"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Only analyze, do not mark for deletion"
    )
    ...
    for schema in schemas:
        ...
        try:
            filtered, stats = analyze_domain_articles(
                conn, schema, limit=args.limit, source_filter=args.source
            )
            all_filtered.extend(filtered)
            all_stats[schema] = stats
            logger.info(
                f"  Found {stats['filtered']} filtered articles out of {stats['total']} total"
            )
