...
class OptimizedMLWorker:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_article):
                ...
                try:
                    ...
                    logger.error(
                        f"Worker {self.worker_id}: Error processing article {article['id']}: {e}"
                    )
                    self.failed_count += 1
    ...
    def process_single_article(self, article):
        ...
        try:
            ...
            logger.info(
                f"Worker {self.worker_id}: Processed article {article_id} (quality: {quality_score:.2f})"
            )
            return True

        except Exception as e:
            logger.error(
                f"Worker {self.worker_id}: Error processing article {article_id}: {str(e)}"
            )
            return False
    ...
    def run_worker_loop(self):
        ...
        logger.info(
            f"Starting optimized ML worker {self.worker_id} with {self.max_workers} threads"
        )
        ...
        while self.is_running:
            try:
                ...
                if articles:
                    logger.info(
                        f"Worker {self.worker_id}: Found {len(articles)} articles to process"
                    )
                    self.process_article_batch(articles)
                    logger.info(
                        f"Worker {self.worker_id}: Batch completed. Processed: {self.processed_count}, Failed: {self.failed_count}"
                    )
                else:
        ...
        logger.info(
            f"Worker {self.worker_id} stopped. Total processed: {self.processed_count}, Failed: {self.failed_count}"
        )
...
def main():
    ...
    worker = OptimizedMLWorker(
        worker_id=args.worker_id, max_workers=args.max_workers, batch_size=args.batch_size
    )
