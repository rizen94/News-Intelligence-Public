...
def main():
    ...
    api_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    path = os.path.join(api_dir, "database", "migrations", "172_daily_analytics_rollups.sql")
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("SET statement_timeout = 0")  # allow CREATE TABLE to complete
            cur.execute(sql)
        conn.commit()
        print("✅ 172_daily_analytics_rollups.sql applied successfully")
