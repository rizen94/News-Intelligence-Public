...
class PredictiveScalingService:
    ...
    async def _train_model(self, historical_data: list[HistoricalDataPoint]):
        ...
        try:
            ...
            logger.info(
                f"Model trained with {len(historical_data)} samples, accuracy: {self.model_accuracy:.3f}"
            )
    ...
    def _extract_features(
        self, current_metrics: dict[str, Any], historical_data: list[HistoricalDataPoint]
    ) -> np.ndarray:
    ...
    async def _make_prediction(
        self, features: np.ndarray, current_metrics: dict[str, Any]
    ) -> LoadPrediction:
    ...
    def _determine_scaling_action(
        ...
        if (
            predicted_cpu > self.scaling_thresholds["cpu_high"]
            or predicted_memory > self.scaling_thresholds["memory_high"]
            or predicted_queue_length > self.scaling_thresholds["queue_high"]
        ):
            # Scale down
            recommended_tasks = max(1, current_parallel_tasks - 1)
            scaling_action = "scale_down"
        elif (
            predicted_cpu < self.scaling_thresholds["cpu_low"]
            and predicted_memory < self.scaling_thresholds["memory_low"]
            and predicted_queue_length < self.scaling_thresholds["queue_low"]
        ):
            # Scale up
