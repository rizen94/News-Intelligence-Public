...
class HistoricalContextService:
    ...
    async def generate_historical_context(
        ...
        try:
            ...
            identified_patterns = await self._identify_historical_patterns(
                storyline_title, historical_timeline
            )

            # Find similar historical events
            similar_events = await self._find_similar_events(
                storyline_title, historical_timeline, articles
            )
    ...
    async def _build_historical_timeline(
    ...
    async def _search_historical_events(
        self,
        storyline_title: str,
            ...
            for i, entity in enumerate(key_entities[:5]):  # Limit to 5 entities
                event = HistoricalEvent(
                    ...
                    metadata={
                        ...
                        "search_topic": key_topics[i % len(key_topics)]
                        if key_topics
                        else "general",
    ...
    async def _identify_historical_patterns(
        self, storyline_title: str, historical_events: list[HistoricalEvent]
            ...
            for pattern_type, pattern_config in self.pattern_templates.items():
                ...
                if matching_events:
                    ...
                    similarity_score = self._calculate_pattern_similarity(
                        matching_events, pattern_config
                    )
                    confidence_level = self._calculate_pattern_confidence(
                        matching_events, similarity_score
                    )
    ...
    async def _find_similar_events(
        self, storyline_title: str, historical_events: list[HistoricalEvent], articles: list[dict]
    ) -> list[HistoricalEvent]:
    ...
    async def _analyze_precedents(
        self, similar_events: list[HistoricalEvent], patterns: list[HistoricalPattern]
    ) -> dict[str, Any]:
            ...
            for pattern in patterns:
                ...
                if pattern.confidence_level > 0.7:
                    if "crisis" in pattern.pattern_type or "conflict" in pattern.pattern_type:
                        precedent_analysis["risk_factors"].append(
                            f"High confidence pattern: {pattern.description}"
                        )
                    else:
                        precedent_analysis["success_factors"].append(
                            f"Positive pattern: {pattern.description}"
                        )
    ...
    async def _analyze_trends(
        self, historical_events: list[HistoricalEvent], patterns: list[HistoricalPattern]
    ) -> dict[str, Any]:
    ...
    def _find_matching_events(
        self, events: list[HistoricalEvent], pattern_config: dict[str, Any]
    ) -> list[HistoricalEvent]:
    ...
    def _calculate_pattern_similarity(
        ...
        for event in events:
            event_text = (event.title + " " + event.description).lower()
            keyword_matches = sum(
                1 for keyword in pattern_config["keywords"] if keyword.lower() in event_text
            )
    ...
    def _calculate_pattern_confidence(
        self, events: list[HistoricalEvent], similarity_score: float
    ) -> float:
    ...
    async def _generate_precedent_analysis(
        self, pattern_type: PatternType, events: list[HistoricalEvent], storyline_title: str
    ) -> str:
    ...
    def _extract_lessons_learned(
        self, events: list[HistoricalEvent], pattern_type: PatternType
    ) -> list[str]:
    ...
    def _calculate_event_similarity(
        self, event: HistoricalEvent, characteristics: dict[str, Any]
    ) -> float:
        ...
        if event_entities and current_entities:
            entity_overlap = len(event_entities.intersection(current_entities)) / len(
                event_entities.union(current_entities)
            )
