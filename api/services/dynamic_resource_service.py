    ...
    async def allocate_resources_dynamically(self) -> ResourceAllocation:
        ...
        try:
            ...
            new_allocation = ResourceAllocation(
                max_parallel_tasks=max(1, int(pattern['parallel_tasks'] * min(cpu_factor, memory_factor))),
                max_memory_usage_gb=max(2.0, self.current_allocation.max_memory_usage_gb * min(cpu_factor, memory_factor)),
                cpu_threshold=max(60.0, 90.0 - (metrics.cpu_percent * 0.5)),  # Lower threshold if CPU is high
                memory_threshold=max(70.0, 95.0 - (metrics.memory_percent * 0.5)),  # Lower threshold if memory is high
            ...
            logger.info(f"Resource allocation updated for {load_level} load: {new_allocation.max_parallel_tasks} parallel tasks")
