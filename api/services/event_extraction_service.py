...
- Only extract events the article itself reports as having happened or being scheduled.
- Analysis, profile, commentary, and opinion pieces: do NOT invent discrete policy/economic outcomes
  (rate decisions, votes, appointments, market moves) that the article does not explicitly state occurred.
- Speculative language ("may", "could", "likely", "if nominated") is NOT an occurred event.
...
_ANALYSIS_PROFILE_TITLE_RE = re.compile(
    r"(?i)\b("
    r"may be|could be|might be|likely to|opinion|op-?ed|analysis|commentary|"
    r"profile|explainer|explained|what .+ means|adult in the room|"
    r"who is|why .+ matters|column|editorial"
    r")\b"
)

_ANALYSIS_PROFILE_BODY_RE = re.compile(
    r"(?i)\b("
    r"in my view|this analysis|this column|opinion piece|guest essay|"
    r"profile of|character study|speculation about"
    r")\b"
)

_POLICY_ECONOMIC_EVENT_TYPES = frozenset(
    {
        "policy_decision",
        "economic_event",
        "market_shift",
...
_OCCURRED_POLICY_CLAIM_RE = re.compile(
    r"(?i)\b("
    r"raises? interest rates?|cuts? interest rates?|rate hike|rate cut|"
    r"federal reserve raises|fed raises|hikes? rates?|slashes? rates?|"
    r"passes? (?:a |the )?bill|signs? (?:a |the )?bill into law|"
    r"declares? (?:a )?national emergency|imposes? sanctions|"
    r"announces? (?:a )?tariff"
    r")\b"
)


def normalize_title_stem(title: str | None, *, max_tokens: int = 8) -> str:
    """Stable content stem for cross-article matching (drop stopwords/punctuation)."""
    tokens = re.findall(r"[a-z0-9]+", (title or "").lower())
    keep = [t for t in tokens if t not in _TITLE_STOPWORDS and len(t) > 1]
    return " ".join(keep[:max_tokens])


def is_analysis_or_profile_source(
    article_title: str | None,
    article_content: str | None = None,
) -> bool:
    """Heuristic: analysis/profile/opinion framing (Fed/Warsh-style pieces)."""
    title = (article_title or "").strip()
    if title and _ANALYSIS_PROFILE_TITLE_RE.search(title):
        return True
    body = (article_content or "")[:2500]
    if body and _ANALYSIS_PROFILE_BODY_RE.search(body):
        return True
    return False


def event_claim_grounded_in_source(
    event_title: str,
    article_title: str | None,
    ...
    *,
    ...
    """True when enough event-title content tokens appear in the source text."""
    stem_tokens = normalize_title_stem(event_title).split()
    if not stem_tokens:
        return False
    haystack = f"{article_title or ''}\n{article_content or ''}".lower()
    if not haystack.strip():
        return True  # no source context available — do not block
    hits = sum(1 for t in stem_tokens if t in haystack)
    need = min(min_token_hits, max(1, len(stem_tokens) // 2))
    return hits >= need


def assess_chrono_event_quality(
    *,
    event_title: str,
    event_type: str,
    temporal_status: str,
    ...
    """
    Return ``ok``, ``downrank``, or ``block`` for chrono identify quality.

    Blocks invented "occurred" policy/economic beats from analysis/profile sources
    (Fed/Warsh pattern). Down-ranks ungrounded policy claims on borderline sources.
    """
    etype = (event_type or "").strip().lower()
    status = (temporal_status or "").strip().lower()
    title = (event_title or "").strip()
    analysis = is_analysis_or_profile_source(article_title, article_content)
    risky_type = etype in _POLICY_ECONOMIC_EVENT_TYPES
    claimy = bool(_OCCURRED_POLICY_CLAIM_RE.search(title))
    grounded = event_claim_grounded_in_source(title, article_title, article_content)

    if analysis and risky_type and status == "occurred" and (claimy or not grounded):
        return "block"
    if analysis and risky_type and status == "occurred":
        return "downrank"
    if risky_type and claimy and status == "occurred" and not grounded:
        return "block"
    if risky_type and not grounded:
        return "downrank"
    return "ok"
...
def compute_event_fingerprint(
    ...
    When the title stem is long enough, prefer type+title+date over actors so
    LLM actor variance does not mint divergent fingerprints for the same beat.
    """
    title_stem = normalize_title_stem(title)
    actor_names = sorted(a.get("name", "").strip().lower() for a in (key_actors or []) if a)
    if title_stem and len(title_stem.split()) >= 3:
        parts = [
...
class EventExtractionService:
    ...
    async def extract_events_from_article(
        ...
        *,
        ...
        Chrono phase identifies / RAG / tags only — ``storyline_id`` is ignored
        and stored empty (storyline attach is a later phase).
        """
        ...
        for idx, raw_evt in enumerate(events_raw):
            ...
            try:
                evt = self._normalise_event(
                    ...
                    article_id,
                    pub_date,
                    None,  # chrono phase must not attach storylines
                    idx,
        ...
        for aid, events_raw in events_by_article.items():
            ...
            for idx, raw_evt in enumerate(events_raw):
                if not isinstance(raw_evt, dict):
                    continue
                try:
                    # Find pub_date / title / content for this article_id
                    art_match = next((a for a in articles if a["article_id"] == aid), None)
                    pub_date_for_article = art_match["pub_date"] if art_match else None
                    art_title = (art_match.get("title") if art_match else None) or None
                    art_content = (art_match.get("content") if art_match else None) or None
    ...
    def _normalise_event(
        ...
        *,
        ...
        ``storyline_id`` is accepted for call-site compatibility but always stored
        empty — chrono identify must not attach events into storylines.
        """
        ...
        quality = assess_chrono_event_quality(
            event_title=title,
        ...
        if quality == "block":
            logger.info(
                "chrono_hallucination_block article_id=%s title=%r type=%s status=%s",
                article_id,
                title[:120],
                event_type,
                temporal_status,
        ...
        if quality == "downrank":
            ...
            logger.info(
                "chrono_hallucination_downrank article_id=%s title=%r type=%s",
                article_id,
                title[:120],
                event_type,
            )

        # Intentionally ignore storyline_id — empty is correct for chrono phase.
        _ = storyline_id
    ...
    def _find_cross_article_canonical(
        self,
        cursor,
        *,
        ...
        """Locate an earlier spine row for the same real-world event (other article)."""
        if fingerprint:
            cursor.execute(
                """
                SELECT id
                ...
                WHERE event_fingerprint = %s
                  AND id <> %s
                  AND (source_article_id IS DISTINCT FROM %s)
                  AND canonical_event_id IS NULL
                ORDER BY extraction_timestamp ASC NULLS LAST, id ASC
                LIMIT 1
                """,
                (fingerprint, new_id, source_article_id),
            )
            row = cursor.fetchone()
            if row:
                return int(row[0])

        stem = normalize_title_stem(title)
        if stem and len(stem.split()) >= 3 and event_type:
            cursor.execute(
                """
                SELECT id, title
                FROM public.chronological_events
                WHERE event_type = %s
                  ...
                  AND (
                        ...
                        OR actual_event_date BETWEEN (%s::date - 3)
                                                 AND (%s::date + 3)
                  )
                ORDER BY extraction_timestamp ASC NULLS LAST, id ASC
                LIMIT 40
            ...
            for cand_id, cand_title in cursor.fetchall() or []:
                if normalize_title_stem(cand_title) == stem:
                    return int(cand_id)
        return None

    def _link_cross_article_duplicate(
        self,
        cursor,
        *,
        new_id: int,
        ...
        """Point new row at canonical cluster root and bump corroboration count."""
        root = int(canonical_id)
        # Follow existing canonical pointer one hop if present
        cursor.execute(
            """
        ...
        cursor.execute(
            ...
            SET canonical_event_id = %s,
                event_cluster_id = COALESCE(event_cluster_id, %s)
            WHERE id = %s
            """,
            (root, root, new_id),
        )
        cursor.execute(
            ...
            SET event_cluster_id = COALESCE(event_cluster_id, %s),
                source_count = COALESCE(source_count, 1) + 1,
    ...
    async def save_events(
        ...
        *,
        ...
        Cross-article duplicates (same fingerprint or strong title stem) are still
        inserted as per-article evidence rows, then immediately clustered onto the
        earliest canonical root so spine consumers see one primary event.
        """
        if not events:
            return 0
        ...
        for idx, evt in enumerate(events):
            ...
            # Chrono writer must never attach storylines.
            evt["storyline_id"] = ""
            row = dict(evt)
            ...
            new_id_holder: dict[str, int | None] = {"id": None}
            ...
            def _insert() -> None:
                ...
                got = cursor.fetchone()
                new_id_holder["id"] = int(got[0]) if got else None

            ok = run_in_savepoint(cursor, conn, f"evt_save_{idx}", _insert)
            if not ok:
                logger.error(
            ...
            if new_id is None:
                # Conflict / no row — still count as handled for this article fingerprint
                saved += 1
                continue

            saved += 1
            try:
                canonical = self._find_cross_article_canonical(
                    cursor,
                    fingerprint=row.get("event_fingerprint"),
                    title=row.get("title"),
                    event_type=row.get("event_type"),
                    actual_event_date=row.get("actual_event_date"),
                    new_id=new_id,
                    source_article_id=row.get("source_article_id"),
                )
                if canonical is not None and canonical != new_id:
                    self._link_cross_article_duplicate(
                        cursor, new_id=new_id, canonical_id=canonical
                    )
                    clustered += 1
            except Exception as e:
                logger.debug(
                    "cross-article cluster link skipped id=%s: %s", new_id, e
                )
        ...
        cursor.close()
        logger.info(
            "Saved %s/%s events to database (cross_article_clustered=%s)",
            saved,
            len(events),
            clustered,
        )
