...
class EarlyQualityService:
    ...
    def _evaluate_quality_threshold(
        ...
        if overall_score < self.quality_threshold:
            rejection_reasons.append(
                f"Overall score {overall_score:.2f} below threshold {self.quality_threshold}"
            )
        ...
        if source_reliability_score < 0.3:
            rejection_reasons.append(
                f"Source reliability too low (score: {source_reliability_score:.2f})"
            )
    ...
    def adjust_quality_threshold(self, current_volume: int, system_load: float) -> None:
        ...
        self.quality_threshold = max(
            0.1, min(0.8, self.quality_threshold)
        )  # Clamp between 0.1 and 0.8

        logger.info(
            f"Adjusted quality threshold to {self.quality_threshold:.2f} (volume: {current_volume}, load: {system_load:.2f})"
        )
    ...
    async def batch_validate_articles(self, articles: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Quality validation complete: {len(passing_articles)} passing, {len(failing_articles)} failing"
            )
