...
async def run_enhancement_cycle(
    ...
    try:
        ...
        result["fact_change_log_processed"] = await loop.run_in_executor(None, lambda: process_fact_change_log(batch_size=fact_batch))
        result["story_update_queue_processed"] = await loop.run_in_executor(None, lambda: process_story_update_queue(batch_size=queue_batch))
    ...
    try:
        ...
        result["entity_profiles_enriched"] = await loop.run_in_executor(None, lambda: run_enrichment_batch(limit=enrich_limit))
    except Exception as e:
