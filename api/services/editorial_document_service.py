...
async def generate_storyline_editorial(domain: str, limit: int = 10) -> dict[str, Any]:
    ...
    **idle in transaction** for minutes and exhausted the worker pool.
    ...
    try:
        ...
        logger.error("generate_storyline_editorial (list query): %s", e)
        try:
            conn.rollback()
        except Exception:
            pass
    ...
    if list_err is not None:
        return {"success": False, "error": list_err}
    ...
    for row in storylines:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Editorial DB update failed for storyline %s: %s", sid, ue)
                try:
                    conn_u.rollback()
                except Exception:
                    pass
...
async def generate_event_editorial(limit: int = 10) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("generate_event_editorial (list query): %s", e)
        try:
            conn.rollback()
        except Exception:
            pass
    ...
    for row in events:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Event chronicles load failed for event %s: %s", eid, ce)
                try:
                    conn_c.rollback()
                except Exception:
                    pass
            ...
            try:
                ...
                logger.warning("Editorial DB update failed for event %s: %s", eid, ue)
                try:
                    conn_u.rollback()
                except Exception:
                    pass
