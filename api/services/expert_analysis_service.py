...
Provide international insights from multilateral and global governance perspectives.""",
    ...
    async def generate_expert_analysis(
        ...
        try:
            ...
            expert_sources = await self._search_expert_sources(
                storyline_title, expertise_areas, rag_context
            )
            ...
            expert_recommendations = self._generate_expert_recommendations(
                expert_synthesis, expert_analyses
            )
    ...
    def _identify_expertise_areas(
        self, storyline_title: str, articles: list[dict], rag_context: dict[str, Any]
    ) -> list[str]:
        ...
        for area_id, area_config in self.expertise_areas.items():
            ...
            keyword_matches = sum(
                1 for keyword in keywords if keyword.lower() in text_content.lower()
            )
    ...
    async def _search_expert_sources(
        self, storyline_title: str, expertise_areas: list[str], rag_context: dict[str, Any]
    ) -> list[ExpertSource]:
    ...
    async def _generate_expert_analysis_for_source(
        self,
        storyline_id: str,
        storyline_title: str,
        source: ExpertSource,
        ...
        try:
            ...
            analysis_context = self._prepare_analysis_context(
                storyline_title, articles, rag_context
            )
    ...
    async def _synthesize_expert_analyses(
        self, storyline_id: str, expert_analyses: list[ExpertAnalysis], storyline_title: str
    ) -> ExpertSynthesis:
        ...
        try:
            ...
            consensus_analysis = await self._generate_consensus_analysis(
                expert_analyses, storyline_title
            )
            ...
            synthesis_metadata = {
                ...
                "source_types": list(
                    set(analysis.source.source_type for analysis in expert_analyses)
                ),
                "expertise_areas": list(
                    set(
                        area
                        for analysis in expert_analyses
                        for area in analysis.source.expertise_areas
                    )
                ),
                "credibility_levels": list(
                    set(analysis.source.credibility_level for analysis in expert_analyses)
                ),
                "generated_at": datetime.now(timezone.utc).isoformat(),
            }
    ...
    async def _generate_fallback_expert_analysis(
        self, source: ExpertSource, storyline_title: str
    ) -> str:
...
"""
    ...
    def _generate_methodology_notes(
        self, source: ExpertSource, source_config: dict[str, Any]
    ) -> str:
    ...
    def _extract_supporting_evidence(
        self, articles: list[dict], analysis_content: str
    ) -> list[dict[str, Any]]:
    ...
    async def _generate_consensus_analysis(
        self, expert_analyses: list[ExpertAnalysis], storyline_title: str
    ) -> str:
    ...
    def _calculate_synthesis_quality_score_from_analyses(
        self, expert_analyses: list[ExpertAnalysis]
    ) -> float:
    ...
    def _generate_synthesis_methodology_notes(self, expert_analyses: list[ExpertAnalysis]) -> str:
        ...
        expertise_areas = set(
            area for analysis in expert_analyses for area in analysis.source.expertise_areas
        )
    ...
    def _calculate_credibility_distribution(
        self, expert_analyses: list[ExpertAnalysis]
    ) -> dict[str, int]:
    ...
    def _generate_expert_recommendations(
        self, expert_synthesis: ExpertSynthesis, expert_analyses: list[ExpertAnalysis]
    ) -> list[dict[str, Any]]:
        """Generate expert recommendations"""
        recommendations = []
    ...
    def _calculate_synthesis_quality_score(
        self, expert_synthesis: ExpertSynthesis, expert_analyses: list[ExpertAnalysis]
    ) -> float:
            ...
            try:
                ...
                db.execute(
                    insert_query,
                    {
                        ...
                        "synthesis_metadata": json.dumps(
                            result.expert_synthesis.synthesis_metadata
                        ),
                    },
                )
