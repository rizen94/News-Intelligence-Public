...
def resolve_date(expression: str, anchor: datetime) -> Tuple[Optional[date], str]:
    """
    ...
    m = re.match(
        r'(\d{1,2})\s+(' + '|'.join(MONTHS) + r')[,.]?\s+(\d{4})', expr
    )
    if m:
        try:
    ...
    m = re.match(
        r'(' + '|'.join(MONTHS) + r')\s+(\d{1,2})[,.]?\s+(\d{4})', expr
    )
    if m:
        try:
...
def extract_temporal_expressions(text: str) -> list[str]:
    ...
    patterns = [
        ...
        r'\b[Qq][1-4]\s+\d{4}\b',
        r'\b(?:' + '|'.join(
            m.capitalize() for m in MONTHS if len(m) > 3
        ) + r')\s+\d{1,2},?\s+\d{4}\b',
        r'\b\d{1,2}\s+(?:' + '|'.join(
            m.capitalize() for m in MONTHS if len(m) > 3
        ) + r'),?\s+\d{4}\b',
