...
class RAGRetrievalModule:
        ...
        try:
            ...
            if use_hybrid and self.embedding_model:
                ...
                initial_results = await self._hybrid_search(
                    query, expanded_query, self.config["max_results_initial"], filters
                )
            elif use_semantic and self.embedding_model:
                # Semantic search only
                initial_results = await self._semantic_search(
                    query, self.config["max_results_initial"], filters
                )
            else:
                # Keyword search only (fallback)
                initial_results = await self._keyword_search(
                    expanded_query, self.config["max_results_initial"], filters
                )
    ...
    async def _hybrid_search(
        self, original_query: str, expanded_query: str, max_results: int, filters: dict[str, Any]
    ) -> list[dict[str, Any]]:
                ...
                articles.append(
                    {
                        ...
                        if row["readability_score"]
                        else 0.0,
                        "credibility_score": float(row["credibility_score"])
                        if row["credibility_score"]
                        else 0.0,
                        "sentiment": {
    ...
    def _merge_hybrid_results(
        ...
        try:
            ...
            for i, article in enumerate(keyword_results):
                ...
                if article_id not in merged:
                    ...
                    merged[article_id]["keyword_score"] = max(
                        merged[article_id].get("keyword_score", 0), keyword_score
                    )
            ...
            sorted_articles = sorted(
                merged.values(), key=lambda x: x["relevance_score"], reverse=True
            )
    ...
    def _rerank_results(
        self, query: str, results: list[dict[str, Any]], max_results: int
    ) -> list[dict[str, Any]]:
