...
class SynthesizedArticle:
    ...
    def to_markdown(self) -> str:
        ...
        for i, article in enumerate(self.source_articles[:20], 1):
            lines.append(
                f"{i}. [{article.get('title', 'Unknown')}]({article.get('url', '#')}) - {article.get('source_name', 'Unknown')}"
            )
        lines.append("")
...
class DeepContentSynthesisService:
    ...
    def extract_facts_from_article(
        self, article_id: int, title: str, content: str, url: str, published_at: datetime
    ) -> list[ExtractedFact]:
        """
...
Focus on the most important and newsworthy facts. Extract 5-15 key facts."""
    ...
    def _extract_facts_fallback(
        ...
        for sentence in sentences[:30]:  # Check first 30 sentences
            ...
            if re.search(
                r"\b(announced|said|reported|revealed|confirmed|stated|declared)\b", sentence, re.I
            ):
                facts.append(
                    ExtractedFact(
                        content=sentence,
    ...
    def synthesize_storyline_content(
        ...
        try:
            ...
            logger.debug(
                "Could not load intelligence context for storyline %s: %s", storyline_id, e
            )
    ...
    def synthesize_topic_content(
        ...
        return self._synthesize_from_articles(
            domain=domain, topic=topic_name, articles=articles, depth=depth
        )
    ...
    def _synthesize_from_articles(
        ...
        if intelligence_context:
            ...
            if claims:
                claim_lines = [
                    f"- {c['subject']} {c['predicate']} {c['object']}" for c in claims[:8]
                ]
                parts.append("Key claims:\n" + "\n".join(claim_lines))
            positions = intelligence_context.get("entity_positions", [])
            if positions:
                pos_lines = [
                    f"- {p['entity_name']} on {p['topic']}: {p['position']}" for p in positions[:6]
                ]
    ...
    def _generate_background_section(
        self, topic: str, domain_context: DomainContext, key_terms: dict[str, str]
    ) -> ContentSection:
...
Write in an encyclopedic, neutral tone like Wikipedia. Be informative and educational."""
        ...
        if key_terms:
            ...
            subsections.append(
                ContentSection(title="Key Terms", content=term_content, subsections=[], sources=[])
            )

        return ContentSection(
            title="Background", content=content, subsections=subsections, sources=[]
        )

    def _generate_developments_section(
        self, topic: str, claims: list[ExtractedFact], events: list[ExtractedFact], depth: str
    ) -> ContentSection:
        """Generate section about current developments"""
        # Compile facts into context
        facts_text = "\n".join(
            [f"- {f.content} (Source: {f.source_title})" for f in (claims + events)[:15]]
        )
...
Write in encyclopedic style. Be factual and balanced. Synthesize the information cohesively, don't just list facts."""
        ...
        content = self._generate_llm_content(
            prompt, max_tokens=1200 if depth == "comprehensive" else 600
        )
    ...
    def _generate_data_section(self, topic: str, statistics: list[ExtractedFact]) -> ContentSection:
        ...
        stats_text = "\n".join(
            [f"- {f.content} (Source: {f.source_title})" for f in statistics[:10]]
        )
...
Be precise with numbers. Explain what they mean for non-expert readers."""
    ...
    def _generate_reactions_section(
        self, topic: str, quotes: list[ExtractedFact], opinions: list[ExtractedFact]
    ) -> ContentSection:
        """Generate section about reactions and perspectives"""
        quotes_text = "\n".join([f'- "{f.content}" (Source: {f.source_title})' for f in quotes[:8]])

        opinions_text = "\n".join(
            [f"- {f.content} (Source: {f.source_title})" for f in opinions[:5]]
        )
...
Be balanced and present multiple sides. Attribute statements clearly."""
        ...
        return ContentSection(
            ...
            subsections=[],
            sources=[
                {"title": f.source_title, "url": f.source_url} for f in (quotes + opinions)[:8]
            ],
            facts_used=quotes + opinions,
        )

    def _generate_analysis_section(
        self, topic: str, all_facts: list[ExtractedFact], domain_context: DomainContext, depth: str
    ) -> ContentSection:
...
Be analytical but balanced. Avoid speculation; base analysis on the facts. Note areas of uncertainty."""
        ...
        content = self._generate_llm_content(
            prompt, max_tokens=800 if depth == "comprehensive" else 400
        )

        return ContentSection(
            title="Analysis and Implications", content=content, subsections=[], sources=[]
        )
...
Write in neutral, encyclopedic tone. Be informative and comprehensive."""
    ...
    def _fetch_storyline_with_articles(
        self, schema: str, storyline_id: int
    ...
    def _find_article_clusters(
        self, schema: str, hours: int, min_articles: int
    ) -> list[dict[str, Any]]:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                for i, article in enumerate(articles):
                    ...
                    if len(cluster) >= min_articles:
                        clusters.append(
                            {"topic": article["title"], "articles": cluster, "count": len(cluster)}
                        )
