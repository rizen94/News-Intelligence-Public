...
def _run_scheduled_dossier_compiles(
    ...
    try:
        with conn.cursor() as cur:
            # Pool default statement_timeout (~120s) cancels the full eligibility
            # EXISTS tree (per-domain article_entities). Prefer missing-dossier drain
            # first — that is the ~24k backlog after months of idle — then try refresh.
            ...
            if len(candidates) < max_dossiers:
                remaining = max_dossiers - len(candidates)
                try:
                    ...
                    cur.execute(
                        ...
                        SELECT ep.domain_key, ep.canonical_entity_id
                        FROM intelligence.entity_profiles ep
                        JOIN intelligence.entity_dossiers ed
                          ON ed.domain_key = ep.domain_key
                         AND ed.entity_id = ep.canonical_entity_id
                        WHERE ep.canonical_entity_id IS NOT NULL
                          AND {domain_sql}
                          AND {needs_compile}
                        ...
                        LIMIT %s
                        """,
                        (domain_keys, remaining),
                    )
                    seen = {(d, e) for d, e in candidates}
                    for r in cur.fetchall():
                        if r[1] is None:
                            continue
                        pair = (r[0], r[1])
                    ...
                    logger.warning(
                        "_run_scheduled_dossier_compiles: refresh select skipped: %s",
                        refresh_err,
                    )
    except Exception as e:
        logger.warning("_run_scheduled_dossier_compiles: select failed: %s", e)
    finally:
        try:
            conn.close()
        except Exception:
