...
def sample_resources() -> ResourceSnapshot:
    ...
    if probe_src.startswith("ollama-ps:"):
        # VRAM occupancy is the reliable signal when nvidia-smi is unreachable.
        if gpu_vram is not None:
            gpu_headroom = max(0.0, min(1.0, 1.0 - float(gpu_vram) / 100.0))
        elif int(gpu_raw.get("ollama_models_loaded") or 0) == 0:
            gpu_headroom = 0.95
        else:
            gpu_headroom = 0.55
