...
class ChronologicalEvent:
    ...
    relative_expression: Optional[str]
    source_text: str
...
class TemporalNLPService:
    ...
    def _compile_temporal_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            ...
            'weeks_from_now': re.compile(r'\b(\d+)\s+(weeks?)\s+(from\s+now|hence)\b', re.IGNORECASE),
            'months_from_now': re.compile(r'\b(\d+)\s+(months?)\s+(from\s+now|hence)\b', re.IGNORECASE),
            'years_from_now': re.compile(r'\b(\d+)\s+(years?)\s+(from\s+now|hence)\b', re.IGNORECASE),
            ...
            'month_day_year': re.compile(r'\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),?\s+(\d{4})\b', re.IGNORECASE),
            'day_month_year': re.compile(r'\b(\d{1,2})\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})\b', re.IGNORECASE),
    ...
    def _compile_historical_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            'previous_shutdown': re.compile(r'\b(previous|last)\s+(government\s+)?shutdown\b', re.IGNORECASE),
            'last_time': re.compile(r'\b(last\s+time|previously|earlier)\b', re.IGNORECASE),
            'in_the_past': re.compile(r'\b(in\s+the\s+past|historically|traditionally)\b', re.IGNORECASE),
            'years_ago_shutdown': re.compile(r'\b(\d{4})\s+(government\s+)?shutdown\b', re.IGNORECASE),
            'administration_reference': re.compile(r'\b(previous|last|former)\s+administration\b', re.IGNORECASE),
            'historical_event': re.compile(r'\b(in\s+\d{4}|during\s+\d{4}|back\s+in\s+\d{4})\b', re.IGNORECASE),
        }
        return patterns
    ...
    def _compile_event_indicators(self) -> List[str]:
        ...
        return [
            'happened', 'occurred', 'took place', 'began', 'started', 'ended', 'concluded',
            'announced', 'declared', 'revealed', 'disclosed', 'reported', 'confirmed',
            'launched', 'initiated', 'implemented', 'enacted', 'passed', 'approved',
            'rejected', 'denied', 'blocked', 'prevented', 'stopped', 'halted',
            'increased', 'decreased', 'rose', 'fell', 'grew', 'declined',
            'won', 'lost', 'succeeded', 'failed', 'achieved', 'accomplished'
        ]
    
    def extract_temporal_expressions(self, text: str, anchor_date: date = None) -> List[TemporalExpression]:
    ...
    def _extract_from_sentence(self, sentence: str, anchor_date: date, sentence_idx: int) -> List[TemporalExpression]:
        ...
        for pattern_name, pattern in self.temporal_patterns.items():
            for match in pattern.finditer(sentence):
                ...
                normalized_date, expression_type, confidence, components = self._parse_temporal_expression(
                    raw_expression, pattern_name, anchor_date
                )
    ...
    def _parse_temporal_expression(self, expression: str, pattern_name: str, anchor_date: date) -> Tuple[Optional[date], str, float, Dict[str, Any]]:
        ...
        if pattern_name in ['yesterday']:
            ...
            if match:
                ...
                if anchor_date.month <= months:
                    normalized_date = anchor_date.replace(year=anchor_date.year - 1, month=12 - (months - anchor_date.month))
                else:
            ...
            match = re.search(r'(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),?\s+(\d{4})', expression, re.IGNORECASE)
            if match:
            ...
            try:
                ...
                expression_type = 'absolute'
                components = {'year': normalized_date.year, 'month': normalized_date.month, 'day': normalized_date.day}
                confidence = 0.9
            except:
                normalized_date = None
                expression_type = 'unknown'
    ...
    def extract_chronological_events(self, text: str, article_title: str = "", source: str = "") -> List[ChronologicalEvent]:
        ...
        for para_idx, paragraph in enumerate(paragraphs):
            ...
            for sent_idx, sentence in enumerate(sentences):
                ...
                if self._contains_event_indicator(sentence):
                    ...
                    sentence_temporals = [te for te in temporal_expressions if te.context_sentence == sentence]
                    
                    if sentence_temporals:
                        # Extract event details
                        event = self._extract_event_from_sentence(
                            sentence, sentence_temporals[0], article_title, source, para_idx, sent_idx
                        )
                        if event:
                            events.append(event)
    ...
    def _classify_event_type(self, sentence: str) -> str:
        ...
        elif any(word in sentence_lower for word in ['announced', 'declared', 'revealed', 'disclosed']):
            return 'announcement'
        elif any(word in sentence_lower for word in ['passed', 'approved', 'enacted', 'legislation']):
            return 'legislative'
        elif any(word in sentence_lower for word in ['meeting', 'talks', 'negotiations', 'discussions']):
            return 'diplomatic'
    ...
    def _calculate_importance_score(self, sentence: str, temporal_expression: TemporalExpression) -> float:
        ...
        sentence_lower = sentence.lower()
        if any(word in sentence_lower for word in ['major', 'significant', 'important', 'critical', 'key']):
            score += 0.2
        ...
        if any(word in sentence_lower for word in ['shutdown', 'government', 'congress', 'senate', 'house']):
            score += 0.1
    ...
    def _extract_historical_context(self, sentence: str) -> Optional[str]:
    ...
    def _create_event_description(self, sentence: str, temporal_expression: TemporalExpression) -> str:
