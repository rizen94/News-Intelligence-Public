...
class ArticleProcessingService:
    ...
    async def process_rss_feeds(self, feed_urls: list[str]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Quality gates: {len(quality_passed_articles)}/{len(all_articles)} articles passed ({quality_result['pass_rate']:.1%})"
            )
            ...
            return {"success": False, "error": str(e), "stats": processing_stats}
    ...
    def _clean_html_content(self, html_content: str) -> str:
        ...
        try:
            ...
            for tag in soup.find_all():
                ...
                if tag.string:
                    tag.string = re.sub(
                        r"\[Image\]|\[Video\]|\[Media\]|\[Picture\]|\[Advertisement\]",
                        "",
                        tag.string,
                    )
            ...
            for element in soup.find_all(
                ["h1", "h2", "h3", "h4", "h5", "h6", "p", "ul", "ol", "blockquote", "br"]
            ):
    ...
    async def _save_articles_to_db(self, articles: list[dict[str, Any]]) -> int:
        ...
        for article in articles:
            try:
                ...
                if dedup_result["status"] == "duplicate":
                    duplicate_count += 1
                    logger.info(
                        f"Duplicate article skipped: {article.get('title', 'Unknown')} - {dedup_result['recommendation']}"
                    )
                    continue

                elif dedup_result["status"] == "error":
                    logger.error(
                        f"Deduplication error for article {article.get('title', 'Unknown')}: {dedup_result.get('error', 'Unknown error')}"
                    )
                    continue
                ...
                try:
                    ...
                    logger.debug(
                        f"Content hash: {processed_article.get('content_hash', 'None')[:16]}..."
                    )
                    ...
                    if processed_article.get("source"):
                        cursor.execute(
                            "SELECT id FROM rss_feeds WHERE feed_name = %s",
                        ...
                        if feed_result:
                            ...
                            logger.warning(
                                f"No feed found for source: {processed_article.get('source')}"
                            )
                    ...
                    saved_count += 1
                    logger.debug(
                        f"Successfully saved article: {processed_article.get('title', 'Unknown')}"
                    )

                except Exception as e:
                    logger.error(
                        f"Error saving article {processed_article.get('title', 'Unknown')}: {e}"
                    )
                    if conn:
                        conn.rollback()
                    continue
                ...
                logger.error(
                    f"Error processing article {article.get('title', 'Unknown')} through deduplication: {e}"
                )
                continue

        # Log summary
        logger.info(
            f"Article processing complete: {saved_count} saved, {duplicate_count} duplicates skipped"
        )
        ...
        if storyline_suggestions:
            ...
            for suggestion in storyline_suggestions[:5]:  # Log first 5
                logger.info(
                    f"Storyline suggestion: {suggestion.get('storyline_suggestion', 'N/A')}"
                )
                    ...
                    try:
                        ...
                        if cluster_result["success"]:
                            ...
                            await topic_clustering_service.save_topics_to_database(
                                cluster_result["data"], batch
                            )
                            topic_stats["clusters_created"] += len(
                                cluster_result["data"].get("topics", [])
                            )
                    except Exception as e:
