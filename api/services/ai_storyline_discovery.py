...
class AIStorylineDiscovery:
            ...
            if response.status_code == 200:
                ...
                if EMBEDDING_MODEL not in models and "nomic-embed-text" not in str(models):
                    ...
                    pull_resp = requests.post(
                        f"{OLLAMA_URL}/api/pull", json={"name": EMBEDDING_MODEL}, timeout=300
                    )
    ...
    def generate_embeddings_parallel(
        self, articles: list[ArticleEmbedding], progress_callback=None
    ) -> list[ArticleEmbedding]:
        """
    ...
    def calculate_hybrid_similarity_matrix(self, articles: list[ArticleEmbedding]) -> np.ndarray:
        ...
        temporal_matrix = np.where(
            ...
            np.where(
                ...
                0.95,
                np.where(
                    time_diff_matrix <= SAME_DAY_HOURS,
                    0.85,
                    np.where(
                        time_diff_matrix <= 72,
                        0.65,
                        np.where(
                            time_diff_matrix <= SAME_WEEK_HOURS,
                            0.45,
                            np.where(
                                time_diff_matrix <= 336,
                                0.25,
            ...
            if len(indices) >= min_sz:
                ...
                if len(cluster_articles) > 1:
                    ...
                    for k in range(len(cluster_articles)):
                        for j in range(k + 1, len(cluster_articles)):
                            prox = self.calculate_temporal_proximity(
                                cluster_articles[k].created_at, cluster_articles[j].created_at
                            )
                ...
                is_breaking = bool(
                    avg_sim >= BREAKING_NEWS_THRESHOLD
                    and len(cluster_articles) >= BREAKING_NEWS_MIN_SIZE
    ...
    def generate_storyline_metadata(
...
{{"title": "headline", "description": "one sentence summary"}}"""
        ...
        fallback_title = (
            ...
            if cluster.is_breaking_news
            else cluster.articles[0].title[:60]
        )
        if cluster.common_entities:
    ...
    def _minimal_cluster_from_article_dicts(
        ...
        for a in articles:
            ...
            ae.entities = self.extract_entities(f"{title} {content[:2000]}")
            ae_list.append(ae)
        if not ae_list:
            return None
        ...
        return StorylineCluster(
            cluster_id=0,
            articles=ae_list,
            centroid=np.zeros(768, dtype=np.float64),
            avg_similarity=0.5,
            is_breaking_news=False,
            importance_score=float(min(1.0, len(ae_list) / 10.0)),
            common_entities=common_entities,
            temporal_score=temporal_score,
        )

    def suggest_title_and_description_from_articles(
        self,
        ...
        *,
        ...
        """
        if not articles:
            return {"title": "Emerging story", "description": ""}
        ...
        if use_llm:
            ...
            meta = {
                ...
                "description": self._generate_fast_description(cluster),
            }
        if not meta.get("title") or not str(meta["title"]).strip():
            meta["title"] = self._generate_fast_title(cluster)
        if not meta.get("description") or not str(meta["description"]).strip():
            meta["description"] = self._generate_fast_description(cluster)
        return meta
    ...
    def _cluster_similarity_to_existing(
        ...
        cluster_topic_ids: set[int] | None = None,
    ) -> float:
        ...
        title_overlap = (
            len(title_a & title_b) / len(title_a | title_b) if (title_a or title_b) else 0.0
        )
        ...
        if cluster_topic_ids is not None and existing.get("topic_ids"):
            ...
            if cluster_topic_ids or existing_topics:
                topic_overlap = len(cluster_topic_ids & existing_topics) / len(
                    cluster_topic_ids | existing_topics
                )
        if cluster_topic_ids is not None and existing.get("topic_ids"):
    ...
    def _cluster_matches_existing(
        ...
        for ex in existing_list:
            sim = self._cluster_similarity_to_existing(
                cluster, ex, cluster_topic_ids=cluster_topic_ids
            )
    ...
    def compare_storylines(
        self, storyline1: StorylineCluster, storyline2: StorylineCluster
    ) -> dict[str, Any]:
        """
    ...
    def find_similar_storylines(
        self,
        target: StorylineCluster,
    ...
    def calculate_storyline_similarity_matrix(
        self, storylines: list[StorylineCluster]
    ) -> dict[str, Any]:
        """
                ...
                related_groups.append(
                    {
                        ...
                        "avg_similarity": float(
                            np.mean(
                                [
                                    similarity_matrix[group[k]][group[j]]
    ...
    def detect_storyline_evolution(
        self, storylines: list[StorylineCluster], time_window_hours: int = 48
    ) -> list[dict[str, Any]]:
        """
            ...
            chain = [
                {
                    ...
                    "time_start": min(a.created_at for a in earlier.articles).isoformat()
                    if earlier.articles
                    else None,
                }
            ]
                    ...
                    chain.append(
                        {
                            ...
                            "time_start": min(a.created_at for a in later.articles).isoformat()
                            if later.articles
                            else None,
    ...
    def suggest_storyline_merges(
        self, storylines: list[StorylineCluster], min_similarity: float = 0.6
    ) -> list[dict[str, Any]]:
        """
                    ...
                    suggestions.append(
                        {
                            ...
                            "shared_entities": comparison["shared_entities"],
                            "combined_article_count": len(storylines[i].articles)
                            + len(storylines[j].articles),
    ...
    def discover_storylines(
        ...
        *,
        ...
        if pdf_contexts:
            ...
            logger.info(
                f"[{domain}] Phase 1: Added {len(pdf_contexts)} PDF section contexts for clustering"
            )
        ...
        phase_start = datetime.now()
        logger.info(
            f"[{domain}] Phase 2: Generating embeddings in parallel ({MAX_EMBEDDING_WORKERS} workers)..."
        )
        ...
        logger.info(
            f"[{domain}] Phase 5: LLM for {len(llm_clusters)}, fallback for {len(fallback_clusters)}"
        )
        ...
        if save_to_db:
            ...
            for cluster in clusters[:10]:
                ...
                if existing_id is not None:
                    ...
                    if added:
                        logger.debug(
                            "[%s] Dedup: merged cluster into existing storyline %s (%s articles)",
                            domain,
                            existing_id,
                            added,
                        )
        ...
        logger.info(
            f"[{domain}] Discovery complete: {len(clusters)} clusters, "
