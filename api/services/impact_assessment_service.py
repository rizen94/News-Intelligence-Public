...
Focus on global cooperation, international law, and diplomatic relations."""
    ...
    async def assess_impacts(self, storyline_id: str, articles: List[Dict], 
                           rag_context: Dict[str, Any] = None) -> ComprehensiveImpactResult:
        """
        ...
        try:
            ...
            mitigation_recommendations = self._generate_mitigation_recommendations(dimension_assessments)
            
            # Calculate assessment quality score
            assessment_quality_score = self._calculate_assessment_quality_score(dimension_assessments)
    ...
    async def _assess_dimension_impact(self, storyline_id: str, articles: List[Dict], 
                                     rag_context: Dict[str, Any], dimension: ImpactDimension) -> ImpactAssessment:
        ...
        try:
            ...
            rag_context_str = self._prepare_rag_context(rag_context) if rag_context else "No additional context available"
            ...
            subcategory_impacts = self._extract_subcategory_impacts(impact_description, config['subcategories'])
            ...
            mitigation_strategies = self._generate_mitigation_strategies(impact_description, dimension)
    ...
    async def _generate_fallback_impact_analysis(self, storyline_title: str, articles: List[Dict], 
                                               dimension: ImpactDimension) -> str:
...
""")
    ...
    def _extract_subcategory_impacts(self, impact_description: str, subcategories: List[str]) -> Dict[str, Any]:
        ...
        for subcategory in subcategories:
            ...
            subcategory_keywords = {
                ...
                'governance_effects': ['governance', 'administration', 'bureaucracy', 'institution'],
            ...
            subcategory_impacts[subcategory] = {
                ...
                'keywords_found': [kw for kw in keywords if kw.lower() in impact_description.lower()],
                'description': f"Impact on {subcategory.replace('_', ' ')}"
            }
        
        return subcategory_impacts
    
    def _extract_supporting_evidence(self, articles: List[Dict], impact_description: str) -> List[Dict[str, Any]]:
    ...
    def _generate_mitigation_strategies(self, impact_description: str, dimension: ImpactDimension) -> List[str]:
    ...
    def _calculate_overall_impact_score(self, dimension_assessments: Dict[str, ImpactAssessment]) -> float:
    ...
    def _identify_high_impact_scenarios(self, dimension_assessments: Dict[str, ImpactAssessment]) -> List[Dict[str, Any]]:
        ...
        for dimension, assessment in dimension_assessments.items():
            if assessment.impact_score >= 0.7 or assessment.risk_level in ['high', 'critical']:
                high_impact_scenarios.append({
                    ...
                    'mitigation_priority': 'high' if assessment.risk_level == 'critical' else 'medium'
                })
        
        return high_impact_scenarios
    
    def _generate_risk_assessment(self, dimension_assessments: Dict[str, ImpactAssessment]) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_risk_dimensions': [dim for dim, assessment in dimension_assessments.items() 
                                   if assessment.risk_level in ['high', 'critical']]
        }
    
    def _generate_mitigation_recommendations(self, dimension_assessments: Dict[str, ImpactAssessment]) -> List[Dict[str, Any]]:
        ...
        for strategy in set(all_strategies):  # Remove duplicates
            priority = 'high' if any(dim in high_risk_dimensions for dim in dimension_assessments.keys()) else 'medium'
            recommendations.append({
                'strategy': strategy,
                'priority': priority,
                'applicable_dimensions': [dim for dim, assessment in dimension_assessments.items() 
                                        if strategy in assessment.mitigation_strategies]
            })
        
        return recommendations
    
    def _calculate_assessment_quality_score(self, dimension_assessments: Dict[str, ImpactAssessment]) -> float:
        ...
        for assessment in dimension_assessments.values():
            ...
            strategy_quality = min(len(assessment.mitigation_strategies) / 5, 1.0)  # Strategy factor
            
            overall_quality = (assessment.confidence_level + content_quality + evidence_quality + strategy_quality) / 4
