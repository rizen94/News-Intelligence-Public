...
from services.temporal_nlp_service import ChronologicalEvent, temporal_nlp_service
...
class ChronologicalTimelineService:
    ...
    def extract_chronological_events_from_storyline(
        self, storyline_id: str
    ...
    def _extract_events_from_article(self, article: dict[str, Any]) -> list[ChronologicalEvent]:
        ...
        try:
            ...
            events = self.temporal_nlp.extract_chronological_events(
                content, article.get("title", ""), article.get("source", "")
            )
    ...
    def _enhance_event_with_context(
        self, event: ChronologicalEvent, article: dict[str, Any]
    ) -> ChronologicalEvent:
    ...
    def _store_chronological_events(
        self, storyline_id: str, events: list[ChronologicalEvent]
    ) -> list[ChronologicalEvent]:
        ...
        try:
            ...
            try:
                ...
                for i, event in enumerate(events):
                    ...
                    event_id = (
                        f"{storyline_id}_event_{i}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    )
                ...
                db.commit()
                logger.info(
                    f"Stored {len(stored_events)} chronological events for storyline {storyline_id}"
                )
                return stored_events
    ...
    def _find_event_relationships(self, storyline_id: str, events: list[ChronologicalEvent]):
        ...
        try:
            ...
            sorted_events = sorted(
                [e for e in events if e.actual_event_date], key=lambda x: x.actual_event_date
            )
    ...
    def _calculate_event_relationship(
        self, event1: ChronologicalEvent, event2: ChronologicalEvent
    ) -> dict[str, Any] | None:
    ...
    def _calculate_relationship_strength(
        self, event1: ChronologicalEvent, event2: ChronologicalEvent, time_gap: int
    ) -> float:
    ...
    def reconstruct_timeline_narrative(self, storyline_id: str) -> TimelineReconstruction:
        ...
        try:
            ...
            sorted_events = sorted(
                events,
                key=lambda x: (x["actual_event_date"], x.get("actual_event_time", "00:00:00")),
            )
...
"""
        ...
        for i, event in enumerate(events, 1):
            event_date = (
                event["actual_event_date"].strftime("%B %d, %Y")
                if event["actual_event_date"]
                else "Unknown Date"
            )
            context += f"""
Event {i} - {event_date}:
...
"""
    ...
    def _calculate_coherence_score(self, narrative: str) -> float:
        ...
        indicator_count = sum(
            1 for indicator in temporal_indicators if indicator in narrative.lower()
        )
    ...
    def _store_timeline_reconstruction(self, reconstruction: TimelineReconstruction):
        ...
        try:
            ...
            try:
                ...
                db.commit()
                logger.info(
                    f"Stored timeline reconstruction for storyline {reconstruction.storyline_id}"
                )
