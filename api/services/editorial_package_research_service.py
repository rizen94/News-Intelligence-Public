    ...
    *,
    ...
    if counts["links_added"] == 0:
        try:
            ...
            seeded = int(
                (
                    seed_heuristic_editor_links(
                        package_id, actor="research_heuristic"
                    )
                    or {}
                ).get("seeded")
                or 0
            ...
            logger.warning("research heuristic link seed failed: %s", e)
    ...
    *,
    ...
    if prior_rounds >= max_rounds() and not force and not dry_run:
        ...
        close_package_thin(
            package_id,
            actor="research_llm",
            rationale=f"research max rounds ({max_rounds()}) — closed_thin",
            reason="max_rounds",
            from_modal="research",
        )
    ...
    if validated.get("insufficient_evidence") and not validated.get("attach") and not validated.get(
        ...
        # Prefer seeding a hypothesized graph from existing members over burning
        # empty research rounds, then escaping to a blank Editor.
        ...
        if not dry_run:
            ...
            try:
                ...
                logger.warning(
                    "research insufficient heuristic seed failed package_id=%s: %s",
                    package_id,
                    e,
                )
            ...
            if seeded > 0:
                _merge_package_metadata(
                    package_id,
                    {
                        ...
                        "last_research_change_count": seeded,
                        "last_research_insufficient": True,
                        "last_research_heuristic_links": seeded,
                ...
                try:
                    routed = request_rework(
                        ...
                        note=(
                            f"post-research heuristic links round {round_n} "
                            f"(seeded={seeded})"
                        ),
                        actor="research_llm",
                    ...
                    logger.warning(
                        "research→reduction after heuristic seed failed: %s", e
                    )
                    routed = {"error": str(e)}
                return {
                    "ok": True,
            ...
            if round_n >= max_rounds():
                ...
                _append_summary_decision(
                    ...
                    metadata={"round": round_n, "escape": "max_rounds", "status": "closed_thin"},
                )
                return {
                    "ok": True,
                    "package_id": package_id,
    ...
    if not apply_dry:
        ...
        if route_target == "closed_thin":
            ...
            try:
                ...
                logger.warning("research closed_thin failed: %s", e)
                routed = {"error": str(e)}
        elif route_target == "editor":
            _append_summary_decision(
                package_id,
                action="converged",
                rationale="research↔reduction escape — both zero-change",
                metadata={
                    "round": round_n,
                    "changed": changed,
    ...
    # Entity-seeded research packages → standing knowledge_profile (no Reduction/Editor gate).
    knowledge_merge: dict[str, Any] | None = None
    if not apply_dry and not dry_run:
        try:
            from services.knowledge_profile_service import (
                auto_merge_from_package,
                parse_entity_legacy_seed,
            )

            seed = str(meta.get("legacy_seed") or "")
            if parse_entity_legacy_seed(seed):
                knowledge_merge = auto_merge_from_package(
                    package_id,
                    actor="research_llm",
                    publish=True,
                    regenerate=True,
            ...
            logger.warning(
                "knowledge profile auto-merge failed package=%s: %s",
                package_id,
                e,
            )
            knowledge_merge = {"skipped": True, "reason": "error", "error": str(e)}

    # Shared package evidence brief (parity with Narrative expand product).
    evidence_brief_merge: dict[str, Any] | None = None
    if not apply_dry and not dry_run:
        try:
            from services.editorial_package_evidence_expand_service import (
                merge_research_findings_into_brief,
            )

            evidence_brief_merge = merge_research_findings_into_brief(
                package_id,
                actor="research_llm",
            )
        except Exception as e:
            logger.warning(
                "package evidence brief research parity failed package=%s: %s",
                package_id,
                e,
            )
            evidence_brief_merge = {"skipped": True, "reason": "error", "error": str(e)}
    ...
    return {
        ...
        "evidence_brief_merge": evidence_brief_merge,
    }
