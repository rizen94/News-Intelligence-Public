...
class RSSProcessingModule:
    ...
    async def process_all_feeds(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"RSS processing completed: {processed_count} feeds processed, {error_count} errors"
            )
            ...
            if processed_count > 0:
                ...
                if deduplication_results["success"]:
                    ...
                    logger.error(
                        f"Deduplication failed: {deduplication_results.get('error', 'Unknown error')}"
                    )

            # End pipeline trace
            self.pipeline_logger.end_trace(trace_id=trace_id, success=error_count == 0)
    ...
    async def _process_feed(self, feed: dict[str, Any]) -> None:
        ...
        try:
            ...
            if feed_data.bozo:
                logger.warning(
                    f"Feed parsing warning for {feed['name']}: {feed_data.bozo_exception}"
                )
            ...
            if articles:
                ...
                deduplication_results = await self.deduplication_service.run_deduplication_pipeline(
                    trace_id="feed_deduplication", feed_id=str(feed["id"])
                )

                if (
                    deduplication_results["success"]
                    and deduplication_results["duplicates_found"] > 0
                ):
                    logger.info(
                        f"Feed deduplication: {deduplication_results['duplicates_found']} duplicates found, {deduplication_results['duplicates_merged']} merged"
                    )
