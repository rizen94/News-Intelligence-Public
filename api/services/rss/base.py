...
class BaseRSSService:
        ...
        try:
            ...
            try:
                ...
                feed_result = db.execute(
                    text("SELECT name FROM rss_feeds WHERE id = :feed_id"), {"feed_id": feed_id}
                ).fetchone()
            ...
            try:
                if feed_id:
                    ...
                    active_feeds = db.execute(
                        text("SELECT COUNT(*) FROM rss_feeds WHERE is_active = true")
                    ).fetchone()[0]
    ...
    async def update_filtering_config(
        ...
        try:
            ...
            try:
                ...
                """),
                    {"config_name": config_name, "config_data": json.dumps(config_data)},
                )
                db.commit()
