...
class AdvancedMonitoringService:
    ...
    async def _get_cache_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "cache_avg_size_bytes": float(cache_stats["avg_size_bytes"])
                if cache_stats["avg_size_bytes"]
                else 0,
            }
    ...
    async def _get_processing_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "avg_processing_time": float(processing_stats["avg_processing_time"])
        ...
        try:
            ...
            for alert in self.alerts:
                if (
                    ...
                    and (datetime.now(timezone.utc) - alert.timestamp).total_seconds() < 300
                ):  # 5 minutes
            ...
            # Log alert
            logger.warning(
                f"ALERT [{rule['severity'].value.upper()}] {rule['title']}: {rule['message']}"
            )
    ...
    async def _handle_anomaly(self, anomaly_result: AnomalyDetectionResult):
        ...
        try:
            ...
            alert = Alert(
                ...
                type=AlertType.PERFORMANCE,
                severity=AlertSeverity.WARNING
                if anomaly_result.anomaly_score < 2.0
    ...
    async def get_monitoring_dashboard(self) -> dict[str, Any]:
        ...
        try:
            ...
            recent_alerts = [
                ...
                and (datetime.now(timezone.utc) - alert.timestamp).total_seconds() < 3600
            ]
    ...
    async def record_rss_feed_metrics(
        self,
        feed_id: int,
        feed_name: str,
        success: bool,
        ...
        try:
            ...
            self.prometheus_metrics["rss_feed_success_rate"].labels(
                feed_id=str(feed_id), feed_name=feed_name
            ).set(success_rate)
    ...
    async def record_duplicate_detection(self, duplicates_found: int, algorithm: str):
        ...
        try:
            ...
            if duplicates_found > 0:
                self.prometheus_metrics["articles_duplicates"].labels(algorithm=algorithm).inc(
                    duplicates_found
                )
    ...
    async def update_system_health_metric(self, component: str, health_score: float):
        ...
        try:
            ...
            self.prometheus_metrics["system_health"].labels(component=component).set(health_score)
    ...
    async def _update_prometheus_metrics_from_database(self):
        ...
        try:
            ...
            for row in success_rates:
                self.prometheus_metrics["rss_feed_success_rate"].labels(
                    feed_id=str(row["id"]), feed_name=row["name"]
