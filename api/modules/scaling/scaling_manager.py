...
class ScalingManager:
    ...
    def create_processing_batch(
        self, article_ids: list[int], batch_type: str = "manual", priority: int = 2
    ) -> str:
                    ...
                    json.dumps(
                        {
                            ...
                            "estimated_duration_minutes": len(article_ids)
                            * 2,  # 2 minutes per article
                        }
                    ),
                ),
            )
        ...
        try:
            ...
            for i in range(0, len(articles), chunk_size):
                ...
                for j, (article_id, order) in enumerate(chunk):
                    ...
                    """,
                        ("completed" if success else "failed", error_msg, batch_id, article_id),
                    )
            ...
            logger.info(
                f"Completed batch {batch_id}: {processed_count} processed, {failed_count} failed"
            )
            return {
                "batch_id": batch_id,
    ...
    def _process_article_chunk(self, article_ids: list[int]) -> dict[int, dict[str, Any]]:
        ...
        for article_id in article_ids:
            try:
                ...
                results[article_id] = {"success": False, "error": str(e)}
        ...
        try:
            ...
            """,
                (
                    ...
                    metrics.raw_articles,
                    metrics.processing_articles,
                    metrics.completed_articles,
                    metrics.failed_articles,
                    metrics.total_timeline_events,
                    metrics.active_storylines,
                    metrics.queue_size,
    ...
    def _check_scaling_alerts(self, metrics: ScalingMetrics):
        ...
        if metrics.database_size_bytes > 10 * 1024 * 1024 * 1024:  # 10GB
            alerts.append(
                f"Database size is large: {metrics.database_size_bytes / (1024**3):.2f}GB"
            )
