...
class StorylineAlertService:
    ...
    def check_for_significant_updates(self, thread_id: int) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            time_window = datetime.now() - timedelta(
                hours=self.significance_thresholds["time_window_hours"]
            )
            ...
            """,
                (thread_id, time_window, self.significance_thresholds["article_quality_threshold"]),
            )
    ...
    def create_alert(self, alert_data: dict[str, Any]) -> int | None:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Created storyline alert {alert_id} for thread {alert_data['thread_id']}"
            )
            return alert_id
    ...
    def _create_alert_data(
        self, thread_info: dict[str, Any], articles: list, significance_score: float
    ) -> dict[str, Any]:
        """
