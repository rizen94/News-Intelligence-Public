...
class ContentPrioritizationManager:
    ...
    def process_article_with_priority(
        self, article_data: dict[str, Any], profile_name: str = "default"
    ) -> dict[str, Any]:
        """
        ...
        try:
            self.logger.info(
                f"Processing article with priority: {article_data.get('title', 'Unknown')}"
            )
            ...
            if priority_result["should_collect"]:
                ...
                if thread_matches:
                    ...
                    self._assign_article_to_thread(
                        article_id, best_match["thread_id"], priority_result
                    )
                ...
                # Step 5: Create new story thread if no matches and high priority
                    ...
                    if new_thread and "id" in new_thread:
                        self._assign_article_to_thread(
                            article_id, new_thread["id"], priority_result
                        )
            ...
            self.logger.info(
                f"Article processed with priority: {priority_result['priority_level_name']} "
    ...
    def create_story_thread(
        ...
        try:
            ...
            if "error" not in thread_result:
                ...
                self.logger.info(
                    f"Story thread created successfully: {title} (ID: {thread_result['id']})"
                )
            else:
    ...
    def build_rag_context(
        self, thread_id: int, context_type: str = "historical", max_articles: int = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if "error" not in context_result:
                ...
                self.logger.info(
                    f"RAG context built successfully: {context_type} for thread {thread_id}"
                )
            else:
    ...
    def get_story_threads(
        self, status: str = "active", priority_level_name: str = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def process_existing_articles_priority(
        self, batch_size: int = 50, max_articles: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            self.logger.info(
                f"Starting priority processing of existing articles (batch_size={batch_size})"
            )
            ...
            self.logger.info(
                f"Priority processing completed: {results['priority_assignments_made']} assignments, "
            ...
            return {"error": str(e), "total_processing_time": time.time() - start_time}
            ...
            try:
                ...
                if priority_result["should_collect"]:
                    ...
                    if thread_matches:
                        best_match = max(thread_matches, key=lambda x: x["match_weight"])
                        self._assign_article_to_thread(
                            article_dict["id"], best_match["thread_id"], priority_result
                        )
                    ...
                    # Create new thread if high priority and no matches
                        ...
                        if new_thread and "id" in new_thread:
                            self._assign_article_to_thread(
                                article_dict["id"], new_thread["id"], priority_result
                            )
    ...
    def _store_article_with_priority(
        self, article_data: dict[str, Any], priority_result: dict[str, Any]
    ) -> int:
    ...
    def _assign_article_to_thread(
        self, article_id: int, thread_id: int, priority_result: dict[str, Any]
    ) -> None:
    ...
    def _create_auto_story_thread(
        ...
        try:
            ...
            thread_description = (
                f"Automatically generated thread for {article_data['source']} article"
            )
