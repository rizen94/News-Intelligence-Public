...
class ContentPrioritizationEngine:
    ...
    def calculate_article_priority(
        ...
        try:
                ...
                for rule in self.user_rules[profile_name]:
                    if self._rule_matches(rule, title, content, source, category):
                        ...
                        if rule["action"] == "boost":
                            ...
                            reasoning.append(
                                f"Boosted by {rule['rule_type']}: {rule['rule_value']}"
                            )
                        elif rule["action"] == "suppress":
                            priority_score -= rule_score * rule["weight"]
                            reasoning.append(
                                f"Suppressed by {rule['rule_type']}: {rule['rule_value']}"
                            )
                        elif rule["action"] == "track":
                            priority_score += rule_score * rule["weight"] * 0.5
                            reasoning.append(
                                f"Tracked by {rule['rule_type']}: {rule['rule_value']}"
                            )
            ...
            result = {
                ...
                "priority_level_name": self.priority_levels.get(priority_level_id, {}).get(
                    "name", "unknown"
                ),
                "reasoning": reasoning,
    ...
    def _rule_matches(
        self, rule: dict[str, Any], title: str, content: str, source: str, category: str
    ) -> bool:
    ...
    def _find_story_thread_matches(
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if (
                    ...
                    or keyword.lower() in category.lower()
                ):
                    matches.append(
                        {
                            "thread_id": thread_id,
        ...
        try:
            ...
            for row in cursor.fetchall():
                priority_stats.append(
                    {"name": row[0], "color_hex": row[1], "article_count": row[2]}
                )
