...
class RAGContextBuilder:
            ...
            if self.config.get("auto_tagging_enabled", True):
                try:
                    ...
                    if "error" not in tag_analysis:
                        tag_update_result = self.tagging_service.update_thread_tags(
                            thread_id, tag_analysis
                        )
                        ...
                        self.logger.info(
                            f"Auto-tagging completed for thread {thread_id}: {tag_update_result.get('updates_made', 0)} updates"
                        )
                    else:
                        context["auto_tagging"] = {"enabled": True, "error": tag_analysis["error"]}
                except Exception as e:
                    self.logger.error(f"Auto-tagging failed for thread {thread_id}: {e}")
                    context["auto_tagging"] = {"enabled": True, "error": str(e)}
            else:
    ...
    def _get_thread_info(self, thread_id: int) -> dict[str, Any] | None:
    ...
    def _build_historical_context(
        self, thread_info: dict[str, Any], max_articles: int
    ) -> dict[str, Any]:
    ...
    def _build_related_context(
        ...
        try:
            ...
            related_articles = self._search_related_articles(
                keywords, thread_info["category"], max_articles, exclude_thread_id=thread_info["id"]
            )
    ...
    def _build_background_context(
        ...
        try:
            ...
            background_articles = self._search_background_articles(
                keywords, thread_info["category"], max_articles
            )
    ...
    def _search_historical_articles(
        self, keywords: list[str], category: str, max_articles: int, before_date: datetime
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for keyword in keywords:
                keyword_conditions.append(
                    f"(LOWER(title) LIKE '%{keyword.lower()}%' OR LOWER(content) LIKE '%{keyword.lower()}%')"
                )
    ...
    def _search_related_articles(
        self, keywords: list[str], category: str, max_articles: int, exclude_thread_id: int
    ) -> list[dict[str, Any]]:
        """Search for related articles"""
        conn = None
        try:
    ...
    def _search_background_articles(
        self, keywords: list[str], category: str, max_articles: int
    ) -> list[dict[str, Any]]:
    ...
    def _extract_key_events(self, articles: list[dict[str, Any]]) -> list[str]:
        ...
        try:
            ...
            for article in articles:
                ...
                if any(
                    word in title.lower()
                    for word in ["breakthrough", "discovery", "announcement", "launch", "release"]
                ):
                    events.append(title)
    ...
    def _extract_key_concepts(self, articles: list[dict[str, Any]]) -> list[str]:
        ...
        try:
            ...
            for article in articles:
                ...
                if any(
                    word in title.lower()
                    for word in ["introduction", "guide", "overview", "basics", "fundamentals"]
                ):
    ...
    def _extract_key_milestones(self, articles: list[dict[str, Any]]) -> list[str]:
        ...
        try:
            ...
            for article in articles:
                ...
                if any(
                    word in title.lower()
                    for word in ["breakthrough", "launch", "release", "announcement", "milestone"]
                ):
    ...
    def _find_connection_points(
        self, articles: list[dict[str, Any]], thread_info: dict[str, Any]
    ) -> list[str]:
    ...
    def _log_context_request(
        self, thread_id: int, context_type: str, context: dict[str, Any]
    ) -> None:
