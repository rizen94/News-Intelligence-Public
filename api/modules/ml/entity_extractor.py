...
class LocalEntityExtractor:
    ...
    def extract_entities(
        ...
        try:
            ...
            logger.info(
                f"Entity extraction completed in {processing_time:.2f}s: {len(entities_data)} entities found"
            )
            return result
    ...
    def get_entity_statistics(
        self, extraction_results: list[EntityExtractionResult]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            most_common_types = sorted(
                entity_type_totals.items(), key=lambda x: x[1], reverse=True
            )[:10]
    ...
    def _create_entity_prompt(self, text: str, entity_types: list[str] | None = None) -> str:
