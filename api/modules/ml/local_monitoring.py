...
class LocalMonitoringSystem:
    ...
    def _evict_largest(self):
        ...
        largest_key = max(self.cache_sizes.keys(), key=lambda k: self.cache_sizes[k])
    ...
    def get_system_metrics(self, hours: int = 1) -> list[dict[str, Any]]:
        ...
        try:
            ...
            recent_metrics = [
                asdict(metric) for metric in self.system_metrics if metric.timestamp > cutoff_time
            ]
    ...
    def get_ai_metrics(self, hours: int = 1) -> list[dict[str, Any]]:
        ...
        try:
            cutoff_time = datetime.now() - timedelta(hours=hours)
            recent_metrics = [
                asdict(metric) for metric in self.ai_metrics if metric.timestamp > cutoff_time
            ]
            return recent_metrics
        except Exception as e:
