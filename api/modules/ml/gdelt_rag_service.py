...
class GDELTRAGService:
    ...
    def get_event_timeline(self, query: str, days_back: int = 7) -> dict[str, Any]:
        ...
        try:
            ...
            params = {"query": gdelt_query, "mode": "artlist", "maxrecords": 100, "format": "json"}
            ...
            logger.info(
                f"Retrieved {timeline_data['total_events']} GDELT events for query: {query}"
            )
    ...
    def get_background_context(
        self, keywords: list[str], article_date: str = None
    ) -> dict[str, Any]:
        """
    ...
    def _generate_timeline_summary(self, events: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "average_tone": sum(event.get("tone", 0) for event in events) / len(events)
            if events
            else 0,
        }
    ...
    def _analyze_geographic_distribution(self, events: list[dict]) -> dict[str, Any]:
        ...
        return {
            "countries": [
                {"country": country, "count": count} for country, count in countries.items()
            ],
    ...
    def _identify_key_events(self, events: list[dict]) -> list[dict]:
        ...
        key_events = sorted(
            events, key=lambda x: (x.get("relevance_score", 0), abs(x.get("tone", 0))), reverse=True
        )
    ...
    def _analyze_trends(self, events: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "change_percentage": ((late_count - early_count) / early_count * 100)
            if early_count > 0
            else 0,
        }
