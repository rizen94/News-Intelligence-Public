...
class LocalAdvancedClustering:
    ...
    def cluster_articles(
        ...
        try:
            ...
            clusters = self._create_cluster_results(
                articles, cluster_labels, embeddings, selected_model
            )
            ...
            logger.info(
                f"Clustering completed in {processing_time:.2f}s: {len(clusters)} clusters found"
            )
            return result
...
"""
    ...
    def _perform_clustering(
        self, embeddings: np.ndarray, algorithm: str, num_clusters: int
    ) -> np.ndarray:
    ...
    def _analyze_cluster_content(
        self, articles: list[dict[str, Any]], model: str
    ) -> tuple[list[str], str]:
        """Analyze cluster content to generate keywords and summary"""
        try:
            # Combine all article texts
            combined_text = " ".join([self._prepare_article_text(article) for article in articles])
