...
class LocalReadabilityAnalyzer:
    ...
    def __init__(self, ollama_url: str | None = None):
        ...
        self.READING_SPEEDS = {"slow": 150, "average": 200, "fast": 250}
            ...
            if json_start != -1 and json_end > json_start:
                ...
                return {
                    "overall_quality_score": max(
                        0.0, min(1.0, float(data.get("overall_quality_score", 0.5)))
                    ),
                    ...
                    "completeness_score": max(
                        0.0, min(1.0, float(data.get("completeness_score", 0.5)))
                    ),
                    ...
                    "factual_consistency": max(
                        0.0, min(1.0, float(data.get("factual_consistency", 0.5)))
                    ),
                    "source_reliability": max(
                        0.0, min(1.0, float(data.get("source_reliability", 0.5)))
                    ),
