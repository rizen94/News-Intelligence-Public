...
class BackgroundMLProcessor:
    ...
    @staticmethod
    def _resolve_article_schema(article_id: int) -> str | None:
    ...
    def start_workers(self):
        ...
        for i in range(self.max_workers):
            worker = threading.Thread(
                target=self._worker_loop, name=f"MLWorker-{i + 1}", daemon=True
            )
            worker.start()
            self.worker_threads.append(worker)

        # Start queue monitor thread
        monitor = threading.Thread(target=self._queue_monitor, name="MLQueueMonitor", daemon=True)
        monitor.start()
    ...
    def _process_task(self, task: dict[str, Any]):
        ...
        try:
            ...
            result = self._execute_ml_operation(operation_type, article_data, model_name)
            ...
            self._log_processing_completion(
                article_id, operation_type, model_name, start_time, end_time, duration, result
            )
            ...
            # Update article processing status
            self._update_article_processing_status(article_id, "failed", str(e), schema=schema)
    ...
    def _execute_ml_operation(
        self, operation_type: str, article_data: dict, model_name: str
    ) -> dict:
    ...
    def queue_article_for_processing(
        self,
        article_id: int,
    ...
    def _get_article_data(self, article_id: int, schema: str) -> dict | None:
    ...
    def _log_processing_start(
        self, article_id: int, operation_type: str, model_name: str, start_time: float
    ):
    ...
    def _update_queue_status(
        self, queue_id: int, status: str, result: dict = None, error: str = None
    ):
