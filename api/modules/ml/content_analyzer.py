...
class ContentAnalyzer:
    ...
    def extract_metadata(self, content: str, title: str = None) -> Dict[str, any]:
        ...
        try:
            ...
            if re.search(r'\b(der|die|das|und|oder|aber|in|auf|zu|für|von|mit|bei)\b', content.lower()):
                language_indicators.append("german")
            if re.search(r'\b(le|la|les|et|ou|mais|dans|sur|à|pour|de|avec|par)\b', content.lower()):
    ...
    def validate_content_quality(self, content: str, title: str = None) -> Dict[str, any]:
        ...
        try:
            ...
            if len(content) < self.min_content_length:
                ...
                quality_assessment["recommendations"].append(f"Content too short (minimum {self.min_content_length} characters)")
            
            # Check maximum length
            if len(content) > self.max_content_length:
                quality_assessment["issues"].append("too_long")
                quality_assessment["recommendations"].append(f"Content too long (maximum {self.max_content_length} characters)")
            
            # Check for meaningful content
            words = content.split()
            if len(words) < 20:
                quality_assessment["issues"].append("insufficient_words")
                quality_assessment["recommendations"].append("Content has too few words for meaningful analysis")
            ...
            if len(valid_sentences) < 2:
                ...
                quality_assessment["recommendations"].append("Content lacks proper sentence structure")
            ...
            quality_assessment["is_valid"] = quality_score >= 0.5 and len(quality_assessment["issues"]) == 0
