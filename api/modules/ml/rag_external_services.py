...
class WikipediaService:
    ...
    def get_article_summary(self, title: str) -> dict[str, Any] | None:
...
class NewsAPIService:
    ...
    def __init__(self, api_key: str = None):
        ...
        self.session = requests.Session()
        self.session.headers.update(
            {"X-API-Key": self.api_key, "User-Agent": "NewsIntelligenceSystem/1.0"}
        )
    ...
    def get_trending_topics(
        ...
        try:
            ...
            params = {"category": category, "country": "us", "pageSize": min(limit, 100)}
...
class KnowledgeGraphService:
    ...
    def search_entities(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        try:
            ...
            params = {"query": query, "limit": min(limit, 20), "indent": True, "key": self.api_key}
...
class TimelineAnalysisService:
    ...
    def build_story_timeline(
        self, articles: list[dict[str, Any]], story_keywords: list[str]
    ) -> dict[str, Any]:
    ...
    def _group_by_time_periods(
        ...
        """Group events by time periods"""
        periods = {"last_24h": [], "last_week": [], "last_month": [], "older": []}
    ...
    def _identify_evolution_phases(self, events: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if len(events) >= 10:
            ...
            phases = [{"name": "Current Phase", "events": events}]
...
class EntityRelationshipService:
            ...
            for article in articles:
                ...
                for word in words:
                    if len(word) > 2:
                        ...
                        if any(
                            org_word in word.lower()
                            for org_word in ["inc", "corp", "ltd", "company", "group"]
                        ):
                            ...
                            loc_word in word.lower()
                            for loc_word in ["city", "state", "country", "united", "america"]
                        ):
    ...
    def find_related_stories(
        self, entities: dict[str, list[dict[str, Any]]], existing_articles: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
...
class RAGExternalServicesManager:
    ...
    def gather_comprehensive_context(
        self, query: str, story_keywords: list[str] = None, max_results: int = 20
    ) -> dict[str, Any]:
        """Gather comprehensive context from all external services"""
        try:
            logger.info(f"Gathering comprehensive context for query: {query}")

            context = {"query": query, "generated_at": datetime.now().isoformat(), "sources": {}}
            ...
            if story_keywords:
                try:
                    ...
                    if all_articles:
                        timeline = self.timeline_analysis.build_story_timeline(
                            all_articles, story_keywords
                        )
            ...
            try:
                ...
                if all_articles:
                    ...
                    related_stories = self.entity_relationships.find_related_stories(
                        entities, all_articles
                    )
        ...
        try:
            ...
            if "timeline_analysis" in context:
                ...
                summary_parts.append(
                    f"Timeline analysis: {timeline.get('total_events', 0)} events across {len(timeline.get('time_periods', {}))} time periods"
                )
