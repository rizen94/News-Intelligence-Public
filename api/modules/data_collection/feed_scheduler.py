...
class FeedScheduler:
    ...
    def start(self, collection_interval_minutes: int = 30):
        ...
        logger.info(
            f"Feed scheduler started - collecting every {collection_interval_minutes} minutes"
        )
    ...
    def _collect_feeds_job(self):
        ...
        try:
            ...
            if results["feeds_successful"] > 0:
                ...
                logger.info(
                    f"Scheduled collection completed: {results['new_articles']} new articles from {results['feeds_successful']} feeds"
                )
            else:
    ...
    def get_collection_history(self, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            hourly_stats = []
            for row in cursor.fetchall():
                hourly_stats.append({"hour": row[0].isoformat(), "article_count": row[1]})
