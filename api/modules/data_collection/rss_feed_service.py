...
class RSSFeedService:
    ...
    def __init__(self, db_config: dict[str, str]):
        ...
        self.session.headers.update({"User-Agent": "NewsIntelligenceSystem/1.0 (RSS Collector)"})
        ...
        self.feeds = [
            ...
            RSSFeed(
                "CNN Top Stories", "http://rss.cnn.com/rss/edition.rss", "General", "US", "en", 1
            ),
            RSSFeed(
                "Al Jazeera English",
            ...
            # UK News Sources
            RSSFeed(
                "The Guardian", "https://www.theguardian.com/world/rss", "General", "UK", "en", 1
            ),
            RSSFeed(
                "The Telegraph", "https://www.telegraph.co.uk/rss.xml", "General", "UK", "en", 1
            ),
            RSSFeed("Financial Times", "https://www.ft.com/rss/home", "Business", "UK", "en", 1),
            RSSFeed(
                "The Independent", "https://www.independent.co.uk/rss", "General", "UK", "en", 1
            ),
            # Technology News
            ...
            RSSFeed(
                "The Verge", "https://www.theverge.com/rss/index.xml", "Technology", "US", "en", 1
            ),
            ...
            # Specialized Sources
            RSSFeed(
                "Foreign Policy", "https://foreignpolicy.com/feed/", "International", "US", "en", 2
            ),
            RSSFeed(
                "The Economist",
            ...
            RSSFeed(
                "Time Magazine", "https://feeds.feedburner.com/time/world", "General", "US", "en", 2
            ),
        ]
    ...
    def collect_all_feeds(
        self, max_articles_per_feed: int = 50, collection_id: str | None = None
    ) -> dict[str, Any]:
        """
            ...
            for feed in enabled_feeds:
                try:
                    ...
                    if feed_result["success"]:
                        ...
                        collection_results["duplicate_articles"] += feed_result[
                            "duplicate_articles"
                        ]
                        ...
                        # Mark feed as failed
                        progress_tracker.complete_feed(
                            collection_id, feed.name, 0, 0, 0, success=False
                        )
                        progress_tracker.add_error(
                            collection_id, f"{feed.name}: {feed_result['error']}"
                        )
                    ...
                    # Mark feed as failed
                    progress_tracker.complete_feed(collection_id, feed.name, 0, 0, 0, success=False)
        ...
        logger.info(
            f"RSS collection completed: {collection_results['new_articles']} new articles from {collection_results['feeds_successful']} feeds"
        )
    ...
    def collect_feed(self, feed: RSSFeed, max_articles: int = 50) -> dict[str, Any]:
        ...
        try:
            ...
            if feed_data.bozo:
                logger.warning(
                    f"RSS feed {feed.name} has parsing issues: {feed_data.bozo_exception}"
                )
            ...
            if feed.error_count >= feed.max_errors:
                feed.enabled = False
                logger.warning(
                    f"Disabled feed {feed.name} due to {feed.error_count} consecutive errors"
                )
    ...
    def _parse_rss_entry(self, entry: Any, feed: RSSFeed) -> Article | None:
        """
    ...
    def add_custom_feed(
        self, name: str, url: str, category: str, country: str, priority: int = 2
    ) -> bool:
    ...
    def test_feed(self, feed_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not feed_data.entries:
                return {"valid": False, "error": "No entries found in RSS feed", "entries_count": 0}
            ...
            return {"valid": False, "error": str(e), "entries_count": 0}
