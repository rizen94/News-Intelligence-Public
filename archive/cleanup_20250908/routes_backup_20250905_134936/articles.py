...
async def get_articles(
    ...
    try:
        ...
        count_query = f"SELECT COUNT(*) as count FROM articles {where_clause}"
        ...
        articles_query = f"""
            SELECT 
                ...
                summary, quality_score, ml_data, category, language
            FROM articles 
            {where_clause}
            {order_clause}
            LIMIT %s OFFSET %s
        ...
        for row in articles_rows:
            articles.append(Article(
                ...
                sentiment_score=None,  # Not in database yet
                entities_extracted=[],  # Not in database yet
                topics_extracted=[],   # Not in database yet
                key_points=[],         # Not in database yet
                readability_score=None,  # Not in database yet
                engagement_score=None   # Not in database yet
            ))
        ...
        for article in articles:
            article_dict = {
                ...
                'processing_completed_at': article.processing_completed_at.isoformat() if article.processing_completed_at else None,
                'summary': article.summary,
                'quality_score': article.quality_score,
                'ml_data': article.ml_data if article.ml_data else {},
                'category': article.category,
                'sentiment_score': article.sentiment_score,
                'entities_extracted': article.entities_extracted if article.entities_extracted else [],
                'topics_extracted': article.topics_extracted if article.topics_extracted else [],
                'key_points': article.key_points if article.key_points else [],
        ...
        error_response = create_error_response(
            ...
            error_code="ARTICLES_FETCH_ERROR",
            details={"page": page, "per_page": per_page}
        )
        
        # Convert to dict and handle datetime serialization
        error_dict = error_response.dict()
        error_dict['timestamp'] = error_dict['timestamp'].isoformat() if error_dict['timestamp'] else None
