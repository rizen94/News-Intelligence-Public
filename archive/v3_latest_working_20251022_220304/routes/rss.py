...
class RSSFeedCreate(RSSFeedBase):
    ...
    language: str = Field("en", max_length=10, description="Feed language")
    ...
    status: str = Field("active", max_length=20, description="Feed status")
    update_frequency: int = Field(30, ge=5, le=1440, description="Update frequency in minutes")
    max_articles_per_update: int = Field(50, ge=1, le=1000, description="Maximum articles per update")
...
class RSSFeed(RSSFeedBase):
    ...
    tier: int = Field(..., description="Feed tier")
    priority: int = Field(..., description="Processing priority")
    language: str = Field(..., description="Feed language")
    ...
    max_articles_per_update: int = Field(..., description="Maximum articles per update")
    success_rate: float = Field(0.0, description="Success rate percentage")
    avg_response_time: int = Field(0, description="Average response time in seconds")
...
async def get_rss_feeds(
    ...
    try:
        ...
        feeds_query = f"""
            SELECT 
                id, name, url, category, is_active, last_fetched, last_success,
