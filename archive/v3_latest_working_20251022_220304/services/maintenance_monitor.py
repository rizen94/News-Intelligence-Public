...
from api.config.paths import PROJECT_ROOT, LOGS_DIR, WEB_DIR, ARCHIVE_DIR, BACKUPS_DIR
...
class MaintenanceMonitor:
    ...
    def check_node_modules_size(self) -> Dict:
        ...
        try:
            if os.path.exists(node_modules_path):
                total_size = sum(f.stat().st_size for f in Path(node_modules_path).rglob('*') if f.is_file())
    ...
    def save_monitoring_report(self, filepath: str = None) -> str:
        ...
        if not filepath:
            filepath = os.path.join(LOGS_DIR, f'maintenance_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
