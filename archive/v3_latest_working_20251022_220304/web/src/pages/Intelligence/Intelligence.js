...
const Intelligence = () => {
  ...
  const fetchIntelligenceData = useCallback(async() => {
    try {
      ...
      const insights = [
        {
          ...
          description: 'Recent articles show a 15% increase in left-leaning content',
          type: 'trend',
          severity: 'medium',
          confidence: 0.85,
          timestamp: new Date(),
      ...
      // Generate trends
      const trends = [
        { name: 'Political Coverage', change: 12.5, direction: 'up', articles: 45 },
        { name: 'Technology News', change: -8.2, direction: 'down', articles: 32 },
        { name: 'Economic Analysis', change: 5.7, direction: 'up', articles: 28 },
        { name: 'International Affairs', change: 15.3, direction: 'up', articles: 38 },
      ];

      // Generate patterns
      const patterns = [
        { pattern: 'Peak Activity', time: '09:00-11:00', frequency: 'Daily', confidence: 0.89 },
        { pattern: 'Source Correlation', sources: ['CNN', 'BBC'], correlation: 0.76 },
        { pattern: 'Topic Clustering', topics: ['Politics', 'Economy'], frequency: 'Weekly' },
      ];

      // Generate anomalies
      const anomalies = [
        { type: 'Volume Spike', description: 'Article volume increased 300%', severity: 'high', timestamp: new Date() },
        { type: 'Source Error', description: 'RSS feed returning errors', severity: 'medium', timestamp: new Date() },
        { type: 'Bias Shift', description: 'Unusual bias pattern detected', severity: 'low', timestamp: new Date() },
      ];

      // Generate recommendations
      const recommendations = [
        { action: 'Add new RSS feeds', priority: 'high', impact: 'Increase source diversity' },
        { action: 'Adjust ML model parameters', priority: 'medium', impact: 'Improve accuracy' },
        { action: 'Update bias detection algorithm', priority: 'low', impact: 'Better bias analysis' },
      ];

      // Generate topic clusters
      const topicClusters = [
        { name: 'Political Coverage', size: 45, keywords: ['election', 'policy', 'government'], sentiment: 'neutral' },
        { name: 'Technology', size: 32, keywords: ['AI', 'tech', 'innovation'], sentiment: 'positive' },
        { name: 'Economy', size: 28, keywords: ['market', 'economy', 'finance'], sentiment: 'negative' },
        { name: 'International', size: 38, keywords: ['global', 'international', 'world'], sentiment: 'neutral' },
      ];

      // Generate entity analysis
      const entityAnalysis = [
        { entity: 'Joe Biden', mentions: 25, sentiment: 'neutral', sources: 8 },
        { entity: 'Donald Trump', mentions: 18, sentiment: 'negative', sources: 6 },
        { entity: 'Federal Reserve', mentions: 12, sentiment: 'negative', sources: 4 },
      ...
      const timelineEvents = [
        ...
        { time: '2024-01-02', event: 'First articles processed', type: 'milestone' },
                  ...
                  <CardContent>
                    ...
                    <Box display="flex" alignItems="center" gap={1} mb={2}>
                      ...
                      <LinearProgress
                        ...
                        sx={{ flex: 1 }}
                      />
                      <Typography variant="caption">{(insight.confidence * 100).toFixed(0)}%</Typography>
                    </Box>
                    <Button
                      size="small"
                  ...
                  secondary={
                    <Box>
                      <Typography variant="body2">
                        ...
                        {pattern.sources && ` | Sources: ${pattern.sources.join(', ')}`}
                        {pattern.topics && ` | Topics: ${pattern.topics.join(', ')}`}
                      </Typography>
                      {pattern.confidence && (
                        <Box mt={1}>
                          <Typography variant="caption">Confidence: {pattern.confidence}</Typography>
                          <LinearProgress
                            variant="determinate"
                  ...
                  <Grid item xs={12} md={6} key={index}>
                    <Paper sx={{ p: 2 }}>
                      ...
                      <Chip
                        ...
                        color={cluster.sentiment === 'positive' ? 'success' :
                          cluster.sentiment === 'negative' ? 'error' : 'info'}
                        size="small"
                      />
                    </Paper>
                  </Grid>
