...
const StorylineReport = () => {
  ...
  const loadStorylineReport = async() => {
    try {
      ...
      try {
        const response = await apiService.get(`/storylines/${id}/report`);
        ...
        console.log('Enhanced API not available, falling back to basic API:', enhancedError);
      }
      ...
      setSources([
        { source: 'CNN', article_count: 5, avg_sentiment: -0.2, avg_quality: 0.85 },
        { source: 'Reuters', article_count: 3, avg_sentiment: 0.1, avg_quality: 0.92 },
        { source: 'BBC', article_count: 2, avg_sentiment: -0.1, avg_quality: 0.88 },
      ]);
  ...
  const handleProcessML = async() => {
    try {
      ...
      const stages = [
        ...
        { stage: 'Analyzing sentiment trends...', progress: 90, duration: 1000 },
      ...
      // Call ML processing API
      const response = await apiService.post(`/storylines/${id}/process-ml`);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3}>
            ...
            <CardContent>
              ...
              {storyline.master_summary ? (
                ...
                <Alert severity="info" sx={{ mb: 2 }}>
                  No AI summary available. Click "Run ML Analysis" to generate one.
                </Alert>
              )}
          ...
          <Card sx={{ mt: 3 }}>
            <CardContent>
              <Box display="flex" alignItems="center" gap={1} mb={2}>
                ...
                <Typography variant="h6">Contributing Articles ({articles.length})</Typography>
              </Box>
                        ...
                        secondary={
                          <Box>
                            ...
                            <Box display="flex" gap={1} mt={1}>
                              <Chip
                                label={`Sentiment: ${(article.sentiment_score || 0).toFixed(2)}`}
                                size="small"
                                color={getSentimentColor(article.sentiment_score)}
                              />
                              <Chip
                                label={`Quality: ${(article.quality_score || 0).toFixed(2)}`}
                                size="small"
                                variant="outlined"
                              />
                            </Box>
        ...
        <Grid item xs={12} md={4}>
                ...
                {sources.map((source, index) => (
                  <ListItem key={index}>
                    <ListItemText
                      ...
                      secondary={`${source.article_count} articles • Avg sentiment: ${source.avg_sentiment.toFixed(2)}`}
                    />
                  </ListItem>
                ))}
              </List>
          ...
          {/* Key Entities */}
          {storyline.key_entities && Object.keys(storyline.key_entities).length > 0 && (
            <Card sx={{ mt: 2 }}>
              <CardContent>
                <Typography variant="h6" gutterBottom>
                  Key Entities
                ...
                <Box display="flex" flexWrap="wrap" gap={1}>
                  {Object.entries(storyline.key_entities).map(([entity, count]) => (
                    <Chip
                      key={entity}
                      label={`${entity} (${count})`}
                      size="small"
              ...
              <Timeline>
                {events.map((event, index) => (
                  <TimelineItem key={event.id}>
                    ...
                    <TimelineSeparator>
                      <TimelineDot color={getSentimentColor(event.sentiment_score)}>
