"""
Enhanced RSS Feed Management Service for News Intelligence System v3.0
Comprehensive feed registry, filtering, and management system
"""
...
class EnhancedRSSService:
    """Enhanced RSS feed management service with comprehensive filtering and monitoring"""
    ...
    async def create_feed(self, feed_config: FeedConfig) -> Dict[str, Any]:
        """Create a new RSS feed in the registry"""
        try:
            db_gen = get_db()
            db = next(db_gen)
            try:
    ...
    async def get_feeds(self, 
        ...
        try:
            ...
            try:
                ...
                query = f"""
                    SELECT id, name, url, description, tier, priority, language, country,
                           category, subcategory, is_active, status, update_frequency,
                           max_articles_per_update, success_rate, avg_response_time,
                           reliability_score, last_fetched, last_success, last_error,
