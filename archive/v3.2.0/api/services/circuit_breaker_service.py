    ...
    async def _execute_with_retry(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.config.retry_attempts + 1):
            ...
            if attempt < self.config.retry_attempts:
                ...
                logger.warning(f"Attempt {attempt + 1} failed for {self.name}, retrying in {delay:.2f}s: {last_exception}")
                await asyncio.sleep(delay)
        
        # All retries failed
        raise CircuitBreakerException(f"All {self.config.retry_attempts + 1} attempts failed for {self.name}", last_exception)
