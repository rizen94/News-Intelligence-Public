...
if ! bash ./scripts/production/enforce_methodology.sh check; then
    print_error "System validation failed"
    print_status "Please fix issues before ending session"
    exit 1
fi

# Quick Python smoke (optional — Docker-based test_pipeline.sh is archived)
print_status "Running quick pytest smoke..."
if command -v uv >/dev/null 2>&1; then
    (cd "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" && PYTHONPATH=api uv run pytest tests/unit/test_orchestrator_types.py -q) || {
        print_warning "Pytest smoke failed (non-blocking for session end)"
    }
else
    print_warning "uv not found — skip pytest smoke"
fi
...
if curl -sf "http://localhost:8000/api/system_monitoring/health" >/dev/null 2>&1; then
    ...
    print_status "See logs/api_server.log and ./start_system.sh"
fi
...
if curl -s "http://localhost:3000" | grep -q -E "root|vite|html"; then
    ...
    print_status "See logs/frontend.log (Vite on port 3000)"
fi
...
## Validation Results
- **System Checks**: $(bash ./scripts/production/enforce_methodology.sh check >/dev/null 2>&1 && echo "✅ Passed" || echo "❌ Failed")
