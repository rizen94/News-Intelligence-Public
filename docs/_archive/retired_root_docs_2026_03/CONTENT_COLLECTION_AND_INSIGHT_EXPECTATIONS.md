...
- The **Monitor → Backlog status progression** table is the source of truth: it shows **Remaining**, **Throughput** (~/hr), and **ETA** for articles, documents, contexts (claims), entity profiles, and storylines. Throughput prefers a **rolling average over the last four days** when enough work completed in that window (`per_hour_source: avg_4d`); otherwise it uses recent 1h/24h measurements or a conservative estimate.
...
- The **backlog_status** API returns **`workload_window_days`** (default 4), **`processed_last_4d`** per lane where applicable, **`per_hour`** / **`per_hour_source`** (`avg_4d`, `measured_1h`, `measured_24h`, `measured_2h` for storylines, or `estimated`), plus **`steady_state`**: `{ ok, checks, reasons }` when automation backlogs are within one batch each (see `backlog_metrics`), monitor SQL backlogs are clear, article inflow trend is not growing, and **`overall_iterations_to_baseline` ≤ 1** (one iteration = one 2h cycle). **Storylines** also include **`synthesis_per_domain_last_1h`**, **`synthesis_per_domain_last_2h`**, and **`synthesis_per_domain_last_4d`**. Use 4d averages for calmer ETAs after enough history exists; use `steady_state.ok` on the Monitor for “everything green” at a glance.
