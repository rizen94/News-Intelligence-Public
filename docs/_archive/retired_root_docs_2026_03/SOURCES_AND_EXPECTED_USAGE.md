...
| **Oil / Gas (prices)** | `commodity_registry.yaml` (default FRED series WTI / Henry Hub) + optional env overrides | FRED-only spot/history (`FRED_API_KEY`); **not** metals.dev; filtered **news** via finance RSS + contexts |
