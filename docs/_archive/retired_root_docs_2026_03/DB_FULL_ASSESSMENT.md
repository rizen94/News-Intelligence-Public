...
| check_data_collection_health | Article counts per domain; `content_enrichment` no success in 24h; `collection_cycle` / `document_processing` / storyline_synthesis / daily_briefing active |
...
| collection_cycle (includes PDF discovery) / document_processing | `intelligence.processed_documents` | New rows or updated extraction JSON |
...
| Doc | Use |
|-----|-----|
| [PIPELINE_DB_ALIGNMENT_REPORT.md](../generated/PIPELINE_DB_ALIGNMENT_REPORT.md) | **Regenerate:** `PYTHONPATH=api uv run python scripts/verify_pipeline_db_alignment.py --write-report docs/generated/PIPELINE_DB_ALIGNMENT_REPORT.md` — pipeline phases vs tables/columns + persistence signals |
