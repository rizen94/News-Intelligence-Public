...
**Chemistry connection phases:** `collision_sampling` (hypothesized proposals), `stimulus_rag` (`rag_evidence_pull_queue`), `protein_harden` (promote + stimulus refinement). Labels: `CHEMISTRY_PHASE_DISPLAY_LABELS` in `monitor_run_vocabulary.py`. Crosswalk: `docs/monitor_alignment/CROSSWALK.md`.
...
**Rows/run (measured vs config):** Each `phase_dashboard` row exposes **`rows_per_run`** (ETA divisor), **`measured_rows_per_run_24h`** (24h AVG from `automation_run_history` batch rows with throughput), **`configured_rows_per_run`** (persisted adaptive batch when tuned, else `BATCH_SIZE_PER_TASK` / env), **`rows_per_run_source`** (`measured_24h*` | `adaptive_persisted` | `config_default` | `no_row_batch_model`), and **`rows_per_run_sample_count`**. Monitor subtitle: measured → `n=… in 24h`; `adaptive_persisted` → `(adaptive)`; `config_default` → `(config)`. SQL aggregation: `monitor_run_vocabulary.query_measured_rows_per_run_by_phase`. Phases without a row-batch model (`collection_cycle`, ops/health) set `rows_per_run_source=no_row_batch_model` and hide ETA runs.
