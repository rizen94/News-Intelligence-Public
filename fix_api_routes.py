#!/usr/bin/env python3
"""
News Intelligence System v3.1.0 - API Routes Consolidation
Fixes duplicate endpoints and ensures schema alignment
"""
...
def analyze_duplicates():
    """Analyze and identify duplicate endpoints"""
    duplicates = {
        'articles': ['articles.py'],
        'story_consolidation': ['story_consolidation.py'],
        'rss_processing': ['rss_processing.py'],
        'automation': ['automation.py'],
        'metrics_visualization': ['metrics_visualization.py'],
        'ml_queue': ['ml_queue.py'],
        'ai_processing': ['ai_processing.py'],
        'rss': ['rss.py'],
        'digest': ['digest.py'],
        'story_management': ['story_management.py'],
        'sources': ['sources.py'],
...
def create_consolidated_routes():
    """Create consolidated route files"""
    
    # Articles routes - consolidate duplicates
    articles_routes = '''
"""
...
'''
    ...
    return articles_routes

def main():
    print("=== API ROUTES CONSOLIDATION ===")
    print("")
    
    duplicates = analyze_duplicates()
    print(f"Found {len(duplicates)} route categories with potential duplicates")
    
    # Create consolidated articles route
    articles_content = create_consolidated_routes()
    
    print("1. Creating consolidated articles route...")
    with open('api/routes/articles_consolidated.py', 'w') as f:
        f.write(articles_content)
    print("   ✅ Consolidated articles route created")
    
    print("")
    print("2. API consolidation recommendations:")
    print("   - Remove duplicate route definitions")
    print("   - Add proper response models to all endpoints")
    print("   - Consolidate similar functionality")
    print("   - Ensure consistent error handling")
    print("   - Add proper validation schemas")
    
    print("")
    print("=== API CONSOLIDATION COMPLETED ===")
