...
def run_checks(conn):
    ...
    lines.append("")
    lines.append("--- Automation phases (last 24h successful runs) ---")
    monitored_phases = [
        "content_enrichment",
        "collection_cycle",
        "document_processing",
    ...
    try:
        cur.execute(
            ...
            GROUP BY phase_name
            """,
            (monitored_phases, last24),
        )
        rows = {r[0]: (r[1], r[2]) for r in cur.fetchall()}
        for p in monitored_phases:
            if p in rows:
                count, last = rows[p]
...
def main():
    ...
    print("=" * 60)
    print("Data collection / pipeline health (as of now)")
    print("=" * 60)
