...
Most `*.sh` entries in this folder are **symlinks** to the canonical scripts under repo `scripts/` (see `archive_deprecated/README.md`). Edit and maintain **`scripts/<name>.sh`**, not copies here.

`restart_api_with_db.sh` is a **wrapper** that runs `scripts/restart_api_with_db.sh`; the obsolete standalone copy is archived under `archive_deprecated/`.
