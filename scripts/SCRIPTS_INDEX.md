...
| Script | Purpose |
|--------|---------|
| `api/scripts/run_migration.py` | **Single-file migrations:** `PYTHONPATH=api uv run python api/scripts/run_migration.py NNN` (resolves `NNN_*.sql`) or `--file exact_name.sql`. Implements apply in `shared/migration_sql_runner.py`. |
...
| `api/scripts/run_migration_189.py` | Migration **189**: `CREATE EXTENSION IF NOT EXISTS pg_trgm` (claim subject → entity fuzzy match). `PYTHONPATH=api uv run python api/scripts/run_migration_189.py` |
| `api/scripts/run_migration_201.py` | Migration **201**: template silos **`politics-2`** / **`finance-2`** (`politics_2`, `finance_2`). Pair with `RSS_INGEST_EXCLUDE_DOMAIN_KEYS` + feed seeding. `PYTHONPATH=api uv run python api/scripts/run_migration_201.py` |
| `api/scripts/run_migration_208.py` | Migration **208**: thin wrapper → `run_migration.py 208` (same as `run_migration_NNN.py` for other single-file SQL). |
| `api/scripts/copy_domain_silo_table_data.py` | Copy rows **politics → politics_2** (or any schema pair with shared tables): read-only on source, writes target only; optional `--replace`; runs **`sync_domain_entity_profiles`** for **`--target-domain-key`**. `PYTHONPATH=api uv run python api/scripts/copy_domain_silo_table_data.py --source-schema politics --target-schema politics_2 --target-domain-key politics-2 --dry-run` |
| `api/scripts/switch_domain_rss_ingest.py` | Turn off **`rss_feeds.is_active`** on one domain and on for another (e.g. **politics → politics-2**). `PYTHONPATH=api uv run python api/scripts/switch_domain_rss_ingest.py --deactivate politics --activate politics-2` |
...
| `api/scripts/init_domain_yaml_from_template.py` | **New domain YAML** from `_template.example.yaml`: `PYTHONPATH=api uv run python api/scripts/init_domain_yaml_from_template.py --domain-key KEY --schema-name SCHEMA --display-name "Name"` — see [DOMAIN_EXTENSION_TEMPLATE.md](../docs/DOMAIN_EXTENSION_TEMPLATE.md). |
| `api/scripts/provision_domain.py` | **New silo:** preflight → SQL → verify; inserts **`data_sources.rss.seed_feed_urls`** into `{schema}.rss_feeds` unless **`--no-seed-rss`**. |
| `api/scripts/seed_domain_rss_from_yaml.py` | **Backfill** `{schema}.rss_feeds` from YAML **`seed_feed_urls`** (e.g. silo created before seeding existed): `PYTHONPATH=api uv run python api/scripts/seed_domain_rss_from_yaml.py --config api/config/domains/KEY.yaml` |
...
| `host_resource_watch.py` | **Long-run CPU/GPU sampling** (htop-like metrics via psutil + optional `nvidia-smi`), **TCP to `DB_HOST:DB_PORT`** via `ss`, optional `--db-stats` (`pg_stat_activity` + `max_connections`), process snapshot, CSV, end-of-run **.env tuning hints**. Run: `uv run python scripts/host_resource_watch.py --minutes 60 --interval 5` |
| `restart_api_and_start_resource_watch.sh` | **Restart uvicorn** + **background 4h watch** (default `MINUTES=240`, `INTERVAL=30`, `--db-stats`, CSV + log under `.local/`). Run: `./scripts/restart_api_and_start_resource_watch.sh` |
...
| `run_resource_router_ramp.py` | **Hybrid CPU/GPU/DB ramp helper:** applies staged `.env` gateway profiles (`baseline` → `step3`) for dynamic router and lane concurrency, optional restart + status pull. Run: `PYTHONPATH=api uv run python scripts/run_resource_router_ramp.py --step baseline --apply [--restart] [--status-url http://localhost:8000/api/system_monitoring/automation/status]`. |
| `../api/scripts/analyze_automation_failures.py` | **Failure grouping:** top `(phase, error snippet)` groups and per-phase success/fail counts — `PYTHONPATH=api uv run python api/scripts/analyze_automation_failures.py [--hours 24]` — see [docs/DIAGNOSTICS_EVENT_COLLECTOR.md](../docs/DIAGNOSTICS_EVENT_COLLECTOR.md) |
...
| `snapshot_backlog_status.sh` | **Burndown snapshots:** saves `backlog_status` + `automation/status` JSON under `.local/backlog_snapshots/`, then prints **one timeline table** for up to **4** snapshots (current + 3 prior), same metrics, oldest→newest, with Δ oldest→now. From repo root: `./snapshot_backlog_status` or `bash scripts/snapshot_backlog_status.sh` — optional `BACKLOG_STATUS_URL` / `AUTOMATION_STATUS_URL`. |
| `backlog_burndown.sh` | **Entrypoint:** `snapshot` (same as above), `timeline` (last 4 `backlog_status_*.json` as one table), `diff` (last two files) or `diff OLD NEW`. |
| `compare_backlog_snapshots.py` | **Pair:** two real paths under `.local/backlog_snapshots/backlog_status_*.json`. **Timeline:** `… timeline` + 2+ paths (chrono order). Use `./scripts/backlog_burndown.sh diff` or `timeline` to avoid typing paths. |
...
| `merge_duplicate_extracted_claims.py` | **Dedupe `intelligence.extracted_claims`** (same context + normalized triple); keeps best confidence; dry-run default, `--apply` to delete in batches. Uses `api/services/extracted_claims_dedupe_service.py` (also AutomationManager phase `extracted_claims_dedupe`). |
| `refresh_claim_subject_gaps.py` | Rebuild `intelligence.claim_subject_gap_catalog` (same as `POST /api/context_centric/claim_subject_gaps/refresh`). |
| `api/scripts/claim_subject_gap_bulk_ignore.py` | **Bulk mark subjects ignored** (per domain) so claims→facts skips them; or `POST /api/context_centric/claim_subject_gaps/bulk_ignore`. |
| `api/scripts/seed_world_entities_from_yaml.py` | **Bulk seed** `{domain}.entity_canonical` + profiles from `api/config/seed_world_entities.yaml` (major orgs/people/countries). Idempotent. `PYTHONPATH=api uv run python api/scripts/seed_world_entities_from_yaml.py` |
| `api/scripts/restcountries_seed.py` | **ISO countries** from REST Countries API → `subject` canonicals + aliases. See `docs/EXTERNAL_ENTITY_SEEDS.md`. |
| `api/scripts/wikidata_sparql_seed.py` | **Wikidata SPARQL** presets (countries, central banks, universities, US Senate) or custom `.rq` file. |
| `api/scripts/seed_entities_from_csv.py` | **CSV** import (`domain_key,name,type,aliases`) → bulk seed. |
| `api/scripts/second_pass_frequent_subjects.py` | **DB second pass:** frequent claim subjects / article entity names → seed `subject` rows. |
| `validate_claims_to_facts.py` | **Validate promote path:** checks `pg_trgm`, recent `versioned_facts` from claims, runs `promote_claims_to_versioned_facts` (optional `--dry-run`). `PYTHONPATH=api uv run python scripts/validate_claims_to_facts.py [--limit 500] [--dry-run]` |
...
| `setup_diagnostics_cron.sh` | Install diagnostics collector **every 4 hours** (`run_diagnostics_collect.py --summary` → `logs/diagnostics_cron.log`) — [docs/DIAGNOSTICS_EVENT_COLLECTOR.md](../docs/DIAGNOSTICS_EVENT_COLLECTOR.md) |
