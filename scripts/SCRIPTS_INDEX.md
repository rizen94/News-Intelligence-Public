...
| `api/scripts/run_migration_208.py` | Migration **208**: **`article_topic_clusters`** on **`politics_2`** / **`finance_2`** (fixes topics route / Route Supervisor). `PYTHONPATH=api uv run python api/scripts/run_migration_208.py` |
...
| `snapshot_backlog_status.sh` | **Burndown snapshots:** saves `backlog_status` + `automation/status` JSON under `.local/backlog_snapshots/`. `bash scripts/snapshot_backlog_status.sh` — optional `BACKLOG_STATUS_URL` / `AUTOMATION_STATUS_URL`. |
| `compare_backlog_snapshots.py` | **Diff two backlog JSON files:** `uv run python scripts/compare_backlog_snapshots.py .local/backlog_snapshots/backlog_status_OLD.json .local/backlog_snapshots/backlog_status_NEW.json` |
