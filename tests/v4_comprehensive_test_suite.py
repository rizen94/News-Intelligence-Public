...
class V4TestSuite:
    ...
    def log_test_result(
        self, test_name: str, passed: bool, details: str, category: str = "general"
    ):
    ...
    def test_database_schema_integrity(self):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                if missing_tables:
                    self.log_test_result(
                        "v4_tables_exist", False, f"Missing tables: {missing_tables}", "database"
                    )
                else:
                    self.log_test_result(
                        "v4_tables_exist", True, f"All {len(v4_tables)} v4 tables exist", "database"
                    )
                ...
                if len(indexes) > 0:
                    ...
                    self.log_test_result(
                        "indexes_exist", False, "No indexes found on v4 tables", "database"
                    )
            ...
            self.log_test_result(
                "database_connection", False, f"Database connection failed: {e}", "database"
            )
    ...
    def test_api_endpoints(self):
        ...
        try:
            ...
            self.log_test_result("health_endpoint", False, f"Health endpoint error: {e}", "api")
        ...
        try:
            ...
            self.log_test_result("articles_endpoint", False, f"Articles endpoint error: {e}", "api")
        ...
        try:
            ...
            self.log_test_result(
                "rss_feeds_endpoint", False, f"RSS feeds endpoint error: {e}", "api"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "data" in data and "storylines" in data["data"]:
                    ...
                    if len(storylines) > 0:
                        ...
                        missing_fields = [
                            field for field in required_fields if field not in storyline
                        ]
            ...
            self.log_test_result(
                "storylines_endpoint", False, f"Storylines endpoint error: {e}", "api"
            )
    ...
    def test_data_consistency(self):
        ...
        try:
            ...
            try:
                ...
                self.log_test_result(
                    "articles_consistency", False, f"API articles error: {e}", "consistency"
                )
            ...
            try:
                ...
                self.log_test_result(
                    "feeds_consistency", False, f"API feeds error: {e}", "consistency"
                )

        except Exception as e:
            self.log_test_result(
                "data_consistency", False, f"Data consistency test failed: {e}", "consistency"
            )
        ...
        for endpoint in endpoints:
            try:
                ...
                if response.status_code == 200 and response_time < 5000:  # Less than 5 seconds
                    self.log_test_result(
                        f"performance_{endpoint.split('/')[-1]}",
                        True,
    ...
    def test_frontend_integration(self):
        ...
        try:
            ...
            self.log_test_result("react_server", False, f"React server error: {e}", "frontend")
        ...
        if os.path.exists(api_service_path):
            ...
            if "api/v4/" in content:
                ...
                self.log_test_result(
                    "api_service_flat", True, "API service uses flat /api paths (no v4)", "frontend"
                )
        else:
            self.log_test_result("api_service_v4", False, "API service file not found", "frontend")
    ...
    def run_all_tests(self):
        ...
        if success:
            ...
            print("✅ V4 architecture is working correctly")
        else:
            print("\n❌ COMPREHENSIVE TEST SUITE FAILED")
            print("⚠️ V4 architecture needs attention")
