...
def test_story_thread_creation():
    ...
    try:
        ...
        if 'error' not in thread_result:
            print(f"   ✓ Story thread created: {thread_result['title']} (ID: {thread_result['id']})")
...
def test_user_interest_rules():
    ...
    try:
        ...
        if 'error' not in rule_result:
            print(f"   ✓ User interest rule added: {rule_result['rule_type']}={rule_result['rule_value']}")
    ...
    try:
        ...
        if threads:
            ...
            if 'error' not in historical_context:
                print(f"   ✓ Historical context built: {historical_context['articles_found']} articles")
...
def test_priority_statistics():
    ...
    try:
        ...
        if 'error' not in priority_stats:
            print(f"   ✓ Priority statistics retrieved successfully")
