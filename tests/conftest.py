...
def clean_test_data(db_connection):
    ...
    with db_connection.cursor() as cur:
        # Clean up test data
        cur.execute(
            "DELETE FROM storyline_articles WHERE storyline_id IN (SELECT id FROM storylines WHERE title LIKE 'Test%')"
        )
...
class TestDataManager:
    ...
    def create_storyline(self, storyline_data):
        ...
        response = self.api.post(
            f"{TestConfig.API_BASE_URL}/api/politics/storylines", json=storyline_data
        )
        if response.status_code == 200:
    ...
    def create_rss_feed(self, feed_data):
        ...
        response = self.api.post(
            f"{TestConfig.API_BASE_URL}/api/politics/rss_feeds", json=feed_data
        )
        if response.status_code == 200:
