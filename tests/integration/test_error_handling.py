...
class TestErrorHandling:
    ...
    def test_invalid_data_handling(self, api_client):
        ...
        response = api_client.post(
            f"{TestConfig.API_BASE_URL}/api/politics/storylines", json={"invalid": "data"}
        )
        ...
        assert response.status_code in [400, 422], (
            f"Expected 400/422 for invalid types, got {response.status_code}"
        )
    ...
    def test_resource_not_found_handling(self, api_client):
        ...
        assert response.status_code == 404, (
            f"Expected 404 for non-existent article, got {response.status_code}"
        )

        # Test non-existent storyline
        response = api_client.get(f"{TestConfig.API_BASE_URL}/api/politics/storylines/99999")
        assert response.status_code == 404, (
            f"Expected 404 for non-existent storyline, got {response.status_code}"
        )

        # Test non-existent topic
        response = api_client.get(
            f"{TestConfig.API_BASE_URL}/api/politics/content_analysis/topics/nonexistent-topic/articles"
        )
        assert response.status_code == 404, (
            f"Expected 404 for non-existent topic, got {response.status_code}"
        )
