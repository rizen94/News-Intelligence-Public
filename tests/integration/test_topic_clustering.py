    ...
    def test_intelligent_word_cloud(self, api_client, test_data_manager):
        ...
        for i in range(5):
            article_data = {
                ...
                "content": "This article discusses technology, innovation, artificial intelligence, machine learning, and data science. It covers topics like programming, software development, and digital transformation.",
                "source_domain": "example.com",
                "published_at": TestConfig.SAMPLE_ARTICLE["published_at"],
            }
            article_id = test_data_manager.create_article(article_data)
        ...
        assert len(found_tech_terms) > 0, (
            f"Should find technology terms, found: {meaningful_words[:10]}"
        )
    ...
    def test_topic_analysis_and_summary(self, api_client, test_data_manager):
        ...
        for i in range(3):
            article_data = {
                ...
                "content": "Artificial intelligence is revolutionizing technology. This article explores AI applications, machine learning algorithms, and the future of intelligent systems.",
                "source_domain": "example.com",
                "published_at": TestConfig.SAMPLE_ARTICLE["published_at"],
            }
            article_id = test_data_manager.create_article(article_data)
        ...
        if topics:
            ...
            # Get topic summary
            response = api_client.get(
                f"{TestConfig.API_BASE_URL}/api/politics/content_analysis/topics/{topic_name}/summary"
            )
            TestUtils.assert_response_success(response)

            summary_data = response.json()["data"]
            TestUtils.assert_data_integrity(
                summary_data, ["summary", "total_articles", "key_points"]
            )

            # Get articles for topic
            response = api_client.get(
                f"{TestConfig.API_BASE_URL}/api/politics/content_analysis/topics/{topic_name}/articles"
            )
            TestUtils.assert_response_success(response)
