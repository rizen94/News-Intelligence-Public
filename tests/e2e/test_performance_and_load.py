    ...
    def test_response_times(self, api_client):
        ...
        for endpoint in endpoints_to_test:
            ...
            assert response.status_code == 200, (
                f"Request failed for {endpoint}: {response.status_code}"
            )
    ...
    def test_large_data_handling(self, api_client, test_data_manager):
        ...
        for i in range(20):
            article_data = {
                ...
                "content": f"This is a test article {i} for testing large dataset handling. "
                * 10,  # Long content
        ...
        response = api_client.get(
            f"{TestConfig.API_BASE_URL}/api/articles", params={"limit": 10, "page": 1}
        )
        TestUtils.assert_response_success(response)
