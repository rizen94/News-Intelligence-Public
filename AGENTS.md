...
| Domain keys | **politics**, **finance**, **artificial-intelligence**, **medicine**, … (from registry) | Politics, FINANCE |
| Feed storage | **rss_feeds** | rssFeeds, RSS Feeds table |
| Content clusters | **topics** | clusters, themes |
| API routes | **`/api/{domain}/...`** (primary), **`/api/...`** (selected globals: context_centric, monitoring, topic management, etc.) | Flat legacy router **retired** — reference copy: `api/archive/legacy_api/` |
...
| **DB backup (homelab)** | `scripts/db_backup_single_latest.sh`, [`docs/DATABASE_BACKUP.md`](docs/DATABASE_BACKUP.md) (single `news_intel_latest.pgdump` on NAS, ~24h RPO) |
...
- **Domains:** **`public.domains`** (active rows) is the authoritative catalog, merged with **`api/config/domains/{domain_key}.yaml`** for prompts and RSS seeds. Onboard new silos via migration + **`provision_domain.py`** — same pipeline and routes once the schema exists; see [`docs/DOMAIN_EXTENSION_TEMPLATE.md`](docs/DOMAIN_EXTENSION_TEMPLATE.md). **`shared.domain_registry`** decides which silos are in the pipeline: RSS **`url_schema_pairs()`**, **`get_all_domains()`** (automation/batch), **`get_schema_names_active()`**. **`public.domains.is_active`** does **not** exclude a silo from RSS/automation (disable via YAML **`is_active: false`** or remove the YAML file). **`data_sources.rss.seed_feed_urls`** in domain YAML are written to **`{schema}.rss_feeds`** by **`api/scripts/provision_domain.py`** (after SQL, unless `--no-seed-rss`) or **`api/scripts/seed_domain_rss_from_yaml.py`** for backfill; silo **`rss_feeds.category`** is NOT NULL — seeding sets **`seed_feed_category`** (default `General`) or use the YAML key under `data_sources.rss`. March 2026 registry/provisioning changes: [`docs/DOMAIN_REGISTRY_AND_PROVISIONING_2026_03.md`](docs/DOMAIN_REGISTRY_AND_PROVISIONING_2026_03.md) (dynamic **`is_valid_domain_key`**, shape-only path pattern, **`run_migration_180.py`** post-SQL hooks, **`shared.services.domain_silo_post_migration`**).
...
2. **Paths:** Use flat `/api` (no version in path). Route path segments: `snake_case` (e.g. `system_monitoring`, `rss_feeds`). Domain-scoped: `/api/{domain}/...` with domain keys from the registry (e.g. `politics`, `finance`, `artificial-intelligence`).
