...
4. **Pool env vars** — `DB_POOL_WORKER_MIN/MAX`, `DB_POOL_UI_MIN/MAX`, `DB_POOL_SA_SIZE/OVERFLOW`. Total must stay under PostgreSQL `max_connections`. Defaults favor **lower worker `MIN`** to avoid many idle sessions; raise `MIN` on high-throughput hosts. **Footprint** = sum over every process (API workers, automation host, scripts) × each pool’s max.
